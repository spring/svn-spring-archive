...
uses
  ...
  SpTBXLists, ImgList, SpTBXjanTracker, SpTBXFormPopupMenu;
...
begin
  ...
  if FMyReadyStatus then MainForm.MiscImageList.GetBitmap(10, ReadyButton.Glyph.Bitmap)
  else MainForm.MiscImageList.GetBitmap(9, ReadyButton.Glyph.Bitmap);
...
//  Label7.Caption := 'Max. wind: ' + Format('%.5g', [MapInfo.MaxWind]);
  ...
  MapImage.Hint := GetMapItem(MapIndex).MapName + ', ' + IntToStr(GetMapItem(MapIndex).MapInfo.Width div 64) + ' x ' + IntToStr(GetMapItem(MapIndex).MapInfo.Height div 64);
end;
...
begin
  ...
  begin
    ...
    MainForm.TryToSendCommand('SAYBATTLEEX', 'suggests ' + Utility.MapList[MapsPopupStringList.ItemIndex]);
  ...
  ChangeMap(MapsPopupStringList.ItemIndex);
...
begin
  ...
  BalanceTeamsButton.Enabled := False;
...
begin
  ...
  BalanceTeamsButton.Enabled := True;
...
begin
  ...
  try
    ...
    begin
      ...
      j := PosEx('RGBCOLOR=', su, i);
      if j = 0 then raise Exception.Create('Corrupt script file!');
      j := j + 9;
      k := PosEx(';', su, j);
      if k = 0 then raise Exception.Create('Corrupt script file!');
      sl := StringParser.ParseString(Copy(s, j, k-j), ' ');
...
begin
  ...
  begin
    ...
    Writeln(f, #9 + #9 + 'countryCode=' + TClient(BattleState.Battle.Clients[i]).Country + ';');
    if TClient(BattleState.Battle.Clients[i]).GetMode = 0 then
    begin
...
//  end;
  ...
  begin
    ...
    begin
      ...
      Writeln(f, #9 + #9 + 'RGBColor=' + FloatToStrF(Misc.ColorToR(PurgedClients[j].TeamColor) / 255, ffFixed, 5, 5)
                                 + ' ' + FloatToStrF(Misc.ColorToG(PurgedClients[j].TeamColor) / 255, ffFixed, 5, 5)
                                 + ' ' + FloatToStrF(Misc.ColorToB(PurgedClients[j].TeamColor) / 255, ffFixed, 5, 5)
                                 + ';');
      Writeln(f, #9 + #9 + 'Side=' + SideList[PurgedClients[j].Side] + ';');
      Writeln(f, #9 + #9 + 'Handicap=' + IntToStr(PurgedClients[j].Handicap) + ';');
      if tmpBool then Writeln(f, #9 + #9 + 'AIDLL=' + AIDLL_FOLDER + '/' + TBot(BattleState.Battle.Bots[PurgedClients[j].ClientIndex]).AIDll + ';');
      Break;
    end;
    Writeln(f, #9 + '}');
  end;
...
begin
  ...
  // open larger minimap:
  MinimapZoomedForm.Image1.Canvas.StretchDraw(Rect(0, 0, MinimapZoomedForm.Image1.Width, MinimapZoomedForm.Image1.Height), GetMapItem(CurrentMapIndex).MapImage.Picture.Bitmap);
...
begin
  ...
  begin
    ...
    ForceSpectatorModeItem.Enabled := (GetClientNodeType(VDTBattleClients.FocusedNode.Index) = NormalClient) and (TClient(BattleState.Battle.Clients[VDTBattleClients.FocusedNode.Index]).GetMode <> 0);
    ...
    begin
      ...
      and (HandicapSpinEditItem.Value <> TClient(BattleState.Battle.Clients[VDTBattleClients.FocusedNode.Index]).GetHandicap) then
        MainForm.TryToSendCommand('HANDICAP', TClient(BattleState.Battle.Clients[VDTBattleClients.FocusedNode.Index]).Name + ' ' + IntToStr(Round(HandicapSpinEditItem.Value)))
      ...
      begin
        ...
        MainForm.TryToSendCommand('UPDATEBOT', TBot(BattleState.Battle.Bots[VDTBattleClients.FocusedNode.Index - BattleState.Battle.Clients.Count]).Name + ' ' + IntToStr(TBot(BattleState.Battle.Bots[VDTBattleClients.FocusedNode.Index - BattleState.Battle.Clients.Count]).BattleStatus) + ' ' + IntToStr(TBot(BattleState.Battle.Bots[VDTBattleClients.FocusedNode.Index - BattleState.Battle.Clients.Count]).TeamColor));
      end;
    end;
  end
...
begin
  ...
  begin
    ...
    if GetClientNodeType(VDTBattleClients.FocusedNode.Index) = NormalClient then
    begin
      MainForm.TryToSendCommand('FORCETEAMNO', TClient(BattleState.Battle.Clients[VDTBattleClients.FocusedNode.Index]).Name + ' ' + IntToStr(team));
...
begin
  ...
  begin
    ...
    if GetClientNodeType(VDTBattleClients.FocusedNode.Index) = NormalClient then
    begin
      MainForm.TryToSendCommand('FORCEALLYNO', TClient(BattleState.Battle.Clients[VDTBattleClients.FocusedNode.Index]).Name + ' ' + IntToStr(ally));
...
begin
  ...
  begin
    if GetClientNodeType(VDTBattleClients.FocusedNode.Index) = NormalClient then
      MainForm.TryToSendCommand('KICKFROMBATTLE', TClient(BattleState.Battle.Clients[VDTBattleClients.FocusedNode.Index]).Name)
    else if GetClientNodeType(VDTBattleClients.FocusedNode.Index) = NormalBot then
      MainForm.TryToSendCommand('REMOVEBOT', TBot(BattleState.Battle.Bots[VDTBattleClients.FocusedNode.Index - BattleState.Battle.Clients.Count]).Name);
  end;
end;
...
begin
  ...
  begin
    ...
    if GetClientNodeType(VDTBattleClients.FocusedNode.Index) = NormalClient then
    begin
      MainForm.TryToSendCommand('FORCETEAMCOLOR', TClient(BattleState.Battle.Clients[VDTBattleClients.FocusedNode.Index]).Name + ' ' + IntToStr(TeamColors[color]));
    end
    else if GetClientNodeType(VDTBattleClients.FocusedNode.Index) = NormalBot then
    begin
      TBot(BattleState.Battle.Bots[VDTBattleClients.FocusedNode.Index - BattleState.Battle.Clients.Count]).TeamColor := TeamColors[color];
      MainForm.TryToSendCommand('UPDATEBOT', TBot(BattleState.Battle.Bots[VDTBattleClients.FocusedNode.Index - BattleState.Battle.Clients.Count]).Name + ' ' + IntToStr(TBot(BattleState.Battle.Bots[VDTBattleClients.FocusedNode.Index - BattleState.Battle.Clients.Count]).BattleStatus) + ' ' + IntToStr(TBot(BattleState.Battle.Bots[VDTBattleClients.FocusedNode.Index - BattleState.Battle.Clients.Count]).TeamColor));
    end;
  end;
end;
...
begin
  ...
  begin
    if GetClientNodeType(VDTBattleClients.FocusedNode.Index) = NormalClient then
    begin
      colorindex := Misc.GetColorIndex(TeamColors, TClient(BattleState.Battle.Clients[VDTBattleClients.FocusedNode.Index]).TeamColor);
    ...
    begin
      colorindex := Misc.GetColorIndex(TeamColors, TBot(BattleState.Battle.Bots[VDTBattleClients.FocusedNode.Index - BattleState.Battle.Clients.Count]).TeamColor);
      if colorindex = -1 then
        TTBXColorPaletteHack(ForceTeamColorPalette).SelectedCell := Point(-1, -1)
      else
        TTBXColorPaletteHack(ForceTeamColorPalette).SelectedCell := Point(colorindex mod ForceTeamColorPalette.ColorSet.ColCount, colorindex div ForceTeamColorPalette.ColorSet.ColCount);
...
begin
  ...
  if (HitInfo.HitNode <> nil) and (GetClientNodeType(HitInfo.HitNode.Index) = NormalClient) then
  begin
    MainForm.OpenPrivateChat(TClient(BattleState.Battle.Clients[HitInfo.HitNode.Index]).Name);
...
begin
  ...
  begin
    if GetClientNodeType(VDTBattleClients.FocusedNode.Index) = NormalClient then
    begin
      MainForm.TryToSendCommand('FORCESPECTATORMODE', TClient(BattleState.Battle.Clients[VDTBattleClients.FocusedNode.Index]).Name);
      AddTextToChat('Forcing spectator mode on ' + TClient(BattleState.Battle.Clients[VDTBattleClients.FocusedNode.Index]).Name + ' ...', Colors.Info, 1);
    end;
  end;
end;
