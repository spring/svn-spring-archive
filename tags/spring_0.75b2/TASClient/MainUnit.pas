{
  ...
  * when adding new notification types, follow these steps:
    * In AddNotificationFormUnit:
      - add description string in the NotifsComboBox
      - add new handler in NotifsComboBoxChange event
      - add new handler in AddButtonClick event
    * In NotificationsUnit:
      - add new identifier to the TNotificationType
      - add new description to NotificationInfo
      - append AddNotification function
      - append FindNotification function
      - append UpdateNotificationList function
...
uses
  ...
  TBX, SpTBXControls, TBXDkPanels, SpTBXFormPopupMenu;
...
const
  VERSION_NUMBER = '0.34'; // Must be float value! (with a period as a decimal seperator)
  ...
  SUPPORTED_SERVER_VERSIONS: array[0..0] of string = ('0.34'); // client can connect ONLY to server with one of these version numbers
  EOL = #13#10;
  ...
  MODS_PAGE_LINK = 'http://spring.unknown-files.net/category/14/Mods/';
  MAPS_PAGE_LINK = 'http://spring.unknown-files.net/category/13/Maps/';
...
var
  ...
  record
    ...
    IgnoreVersionIncompatibility: Boolean; // if true, client will ignore version mismatch of spring exe or lobby client upon connecting to server
  end;
  ...
  Status: record
    MySpringVersion: string; // acquired via unitsync.dll (or "spring.exe /V". Latter is now deprecated)
...
uses BattleFormUnit, PreferencesFormUnit, Misc, WaitForAckUnit,
  ...
  LoginProgressFormUnit, GpIFF, SearchFormUnit;
...
begin
  ...
  try
    ...
    begin
      ...
      MainForm.TryToSendCommand('BANLISTADD', sl[1] + ' ' + tmp);
    end
    ...
    begin
      ...
      MainForm.TryToSendCommand('BANLISTREMOVE', sl[1]);
    end
    else if (sl[0] = 'BANLIST') and (not CameFromBattleScreen) then
    begin
      MainForm.TryToSendCommand('BANLIST');
    end
...
begin
  ...
  try // try..finally
    ...
    begin
      if (sl.Count <> 4) or (not Debug.IgnoreVersionIncompatibility and not CheckServerVersion(sl[1])) then
      begin
      ...
      begin
        AddMainLog('Server says your Spring version (' + Status.MySpringVersion + ') is outdated (server only supports ' + sl[2] + ')', Colors.Info);
        AddMainLog('Requesting update from server ...', Colors.Info);
...
//        MyTeamColorIndex := color;   -> not needed anymore, we should keep last used color anyway
    ...
    begin
      ...
      Battle := GetBattle(i);
      SortBattleInList(Battles.Count-1, Preferences.BattleSortStyle, Preferences.BattleSortDirection = 0);
    ...
    begin
      ...
      UpdateClientsListBox;
      SortBattleInList(Battle.Node.Index, Preferences.BattleSortStyle, Preferences.BattleSortDirection = 0);
      VDTBattles.Invalidate;
    ...
    begin
      ...
      if Client.InBattle then
        ...
        begin
          ...
          SortBattleInList(TBattle(Battles[i]).Node.Index, Preferences.BattleSortStyle, Preferences.BattleSortDirection = 0);
      ...
      if Client.InBattle and BattleForm.IsBattleActive and (BattleState.Battle.Clients.IndexOf(Client) <> -1) then
    ...
    begin
      ...
      begin
        ...
        if Misc.GetColorIndex(TeamColors, teamcolor) <> -1 then
          MyTeamColorIndex := Misc.GetColorIndex(TeamColors, teamcolor);
      end;
...
begin
  ...
  TryToSendCommand('JOINBATTLE', IntToStr(TBattle(Battles[Node.Index]).ID) + IFF(pass <> '', ' ' + pass, ''));
end;
...
begin
  ...
  begin
    if Status.Me = nil then Exit; // shouldn't of happen
...
begin
  ...
  try
    if PageControl1.ActivePage.Caption = LOCAL_TAB then (Sender as TlistBox).Hint := ''
    else (Sender as TlistBox).Hint := TClient((PageControl1.ActivePage as TMyTabSheet).Clients[tmp]).Name + ', ' + GetCountryName(TClient((PageControl1.ActivePage as TMyTabSheet).Clients[tmp]).Country);
  except
  end;
end;
