...
class assistant:
    def __init__(self, setup_profiles):
        ...
        spring_binary_location.connect('changed', self.check_file)
        vbox_1.pack_start(spring_binary_location, True, True, 0)        
            
        self.assistant.append_page(vbox_1)
        self.assistant.set_page_title(vbox_1, 'Spring Excuteable')
        ...
        spring_datadir_location.connect('changed', self.check_directory)
        ...
        unitsync_dir_location = gtk.Entry(max=0)
        unitsync_dir_location.show()
        unitsync_dir_location.connect('changed', self.check_directory)
        vbox_3.pack_start(unitsync_dir_location, True, True, 0)
        
        self.assistant.append_page(vbox_3)
        self.assistant.set_page_title(vbox_3, 'Unitsync Location')
        self.assistant.set_page_type(vbox_3, gtk.ASSISTANT_PAGE_CONTENT)
     
        
        ## Contstruct Page 4
        
        vbox_4 = gtk.VBox(False, 5)
        vbox_4.set_border_width(5)
        vbox_4.show()

        label = gtk.Label("UnityLobby is still under development & multiplayer is not yet in working condition.")
        label.show()
        vbox_4.pack_start(label, True, True, 0)
        
        self.assistant.append_page(vbox_4)
        self.assistant.set_page_title(vbox_4, "Author's Notes")
        self.assistant.set_page_type(vbox_4, gtk.ASSISTANT_PAGE_SUMMARY)
        self.assistant.show()
    ...
    def check_directory(self, entry):
    ...
    def get_profile_values(self):
        if self.assistant.get_current_page() == -1:
            ...
            unitsync_location = ((self.assistant.get_nth_page(3)).get_children()[0]).get_text()
        else:
            profile_name = None
            spring_binary = None
            spring_datadir = None
            unitsync_location = None
        return profile_name, spring_binary, spring_datadir, unitsync_location
...
class profile:
    ...
    def main(self):
        if (len(self.setup_profiles) >= 2 or self.show == True) and self.setup == False:
            ...
            if (os.path.isfile( os.path.join(self.unity_location, 'profiles', 'default.profile')) == True) and (self.setup == False):
                profile_name, spring_binary, spring_datadir, unitsync_location = self.read_default_profile()
            else:
                self.wizard = assistant(self.setup_profiles)
                self.wizard.main()
                profile_name, spring_binary, spring_datadir, unitsync_location = self.wizard.get_profile_values()
    ...
    def read_default_profile(self):
        file = os.path.join(self.unity_location, 'profiles', 'default.profile')
        
        config = ConfigParser.ConfigParser()
        config.optionxform = lambda x: x
        config.read(file)
        
        profile_name = config.get('SETUP', 'PROFILE_NAME')
        spring_datadir = config.get('SETUP', 'SPRING_DATADIR')
        spring_binary = config.get('SETUP', 'SPRING_BINARY')
        unitsync_location = config.get('SETUP', 'UNITSYNC_LOCATION')
