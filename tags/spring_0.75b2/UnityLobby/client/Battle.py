...
class Battle:
    ...
    ## ==================================================================================================
        ...
        self.game_settings = {'battle description': GLOBAL.DESCRIPTION,
                            'locked': GLOBAL.LOCK_BATTLE_DEFAULT,
                            'game mode': GLOBAL.GAME_MODES_DEFAULT, 
                            'limited dgun':GLOBAL.LIMITED_DGUN_DEFAULT, 
                            'starting position': GLOBAL.STARTING_POSITIONS_DEFAULT, 
                            'ghosted Buildings':GLOBAL.GHOSTED_BUILDINGS_DEFAULT, 
                            'metal': GLOBAL.METAL_RESOURCES_DEFAULT, 
                            'energy': GLOBAL.ENERGY_RESOURCES_DEFAULT, 
                            'diminishing MMs':GLOBAL.DIMINISHING_MSS_DEFAULT, 
                            'max units': GLOBAL.MAX_UNITS_DEFAULT, 
                            'battle type': 0, 
                            'nat':0, 
                            'rank':0, 
                            'Spectator Count':0, 
    ...
    def integrate_with_lobby(self, status_icon):
    ## ==================================================================================================
    ...
    def integrate_with_options(self, status_icon):
    ## ==================================================================================================
...
#---GUI Code---#000000#FFFFFF---------------------------------------------------
    ...
    ## ==================================================================================================
        ...
        self.player_treeview.connect('event', self.popup_menu)
        ...
        self.lock_battle_combobox.connect("changed", self.update_battleinfo)
        online_options_table.attach(self.lock_battle_combobox, 1, 2, 3, 4)
                    # Host Battle
        self.host_battle_button = gtk.ToggleButton(label="Host Battle")
        self.host_battle_button_handler_id = self.host_battle_button.connect("toggled", self.network_game)
        ...
        self.game_mode.connect("changed", self.update_battledetails)
        ...
        self.limited_dgun.connect("changed", self.update_battledetails)
        ...
        self.starting_positions.connect("changed", self.update_battledetails)
        ...
        self.ghosted_buildings.connect("changed", self.update_battledetails)
        ...
        self.start_metal_spinner.connect("value-changed", self.update_battledetails)
        ...
        self.start_energy_spinner.connect("value-changed", self.update_battledetails)
        ...
        self.diminishing_mms_combobox.connect("changed", self.update_battledetails)
        ...
        self.max_units_spinner.connect("value-changed", self.update_battledetails)
        ...
        self.gdb_backtrace.connect("changed", self.update_battledetails)
        ...
        map_selection.connect('changed', self.map_selection)
        ...
        mod_selection.connect('changed', self.mod_selection)

        self.mod_selection(mod_selection)        
    ...
    ## ==================================================================================================
        ...
        if event.type == gtk.gdk._2BUTTON_PRESS:
            if event.button == 1:
                ...
                (model, scan_iter) = treeselection.get_selected()
                name = self.player_liststore.get_value(scan_iter, 1)
                ...
                if scan_iter != None:
                    ...
                    change_army_item.set_submenu(menu_change_army)
                                # Army
                    foo = self.player_liststore.get_value(scan_iter, 2)
                    if foo == None:
                        ...
                        for o in range(1, GLOBAL.MAX_TEAMS + 1):
                            ...
                            sub_army_item.connect("button_press_event", self.update_army_value, scan_iter, o)                    
                            sub_army_item.show()
                            if foo == o:
                    ...
                    change_ally_item.set_submenu(menu_change_ally)
                                # Ally
                    foo = self.player_liststore.get_value(scan_iter, 8)
                    if foo == None:
                        ...
                        for o in range(1, GLOBAL.MAX_ALLIES + 1):
                            ...
                            sub_ally_item.connect("button_press_event", self.update_ally_value, scan_iter, o)                                            
                            sub_ally_item.show()
                            if foo == o:
                    ...
                    change_side_item.set_submenu(menu_change_side)
                                # Side
                    foo = self.player_liststore.get_value(scan_iter, 7)
                    if foo == None:
                        change_side_item.set_sensitive(False)
                    else:
                        for o in range(0, len(self.sides)):
                            sub_side_item = gtk.MenuItem(self.sides[o])
                            sub_side_item.connect("button_press_event", self.update_side_value, scan_iter, o)                                            
                            sub_side_item.show()
                            if foo == self.sides[o]:
                    ...
                    change_colour_item.connect("button_press_event", self.choose_color, scan_iter)                    
...
#                    change_handicap_item.connect("button_press_event", self.ChangeHandicap, scan_iter)
                    ...
                    kick_item.connect("button_press_event", self.kick_player_or_bot, scan_iter)                    
                    kick_item.show()                
                    ...
                    switch_to_player_item.connect("button_press_event", self.update_player_spectator_value, scan_iter, 1)
                    ...
                    switch_to_spectator_item.connect("button_press_event", self.update_player_spectator_value, scan_iter, 0)                    
                    switch_to_spectator_item.show()
                    menu_switch_to_item.append(switch_to_spectator_item)
                    
                    # Sensitive Settings
                    if self.online == True:
                        if (self.host == False) and (self.player_liststore.get_value(scan_iter, 1) != self.player_name_online):
                            change_army_item.set_sensitive(False)
                            change_ally_item.set_sensitive(False)
                            change_colour_item.set_sensitive(False)
                            switch_to_item.set_sensitive(False)

                        if (self.player_liststore.get_value(scan_iter, 1) != self.player_name_online) and (self.player_liststore.get_value(scan_iter, 1) != self.player_name_online):
                            change_side_item.set_sensitive(False)                            
                    ...
                    if self.player_liststore.get_value(scan_iter, 1) == self.player_name_online:
                        kick_item.set_sensitive(False)
                        ...
                        if self.player_liststore.get_value(scan_iter, 1) == self.player_name_offline:
                            kick_item.set_sensitive(False)
                            if self.player_liststore.get_value(scan_iter, 2) == None:
                                switch_to_spectator_item.set_sensitive(False)
                            else:
            ...
            if event.button == 3:
                ...
                for i in range(0, len(ai)):
                    ...
                    while p <= ( len(self.sides) - 1 ):
                        ...
                        if self.online == True:
                            select_ai_side_item.connect("button_press_event", self.add_bot, ai[i], None, self.player_name_online, ai_location[i], self.sides[p])
                        else:
                            select_ai_side_item.connect("button_press_event", self.add_bot, ai[i], None, self.player_name_offline, ai_location[i], self.sides[p])
    ...
    def handicap_dialog(self):
        pass
        
#---Battle Player List Code---#000000#FFFFFF------------------------------------
    def add_bot(self, widget, event, ai, ai_name, ai_owner, ai_location, side = None, battlestatus=None, teamcolor=None):
    ## ==================================================================================================
    ##  Add Bot
    ## ==================================================================================================
        ...
        ## Get Values from BattleStatus & Color bits
            # BattleStatus
            ready, team, ally, mode, handicap, sync, side = self.battlestatus_decode(int(battlestatus))
            ready = None
            sync = None
            team = team + 1
            ally = ally + 1
            ...
            # Team Color
            red, green, blue = self.battlestatus_color_decode(int(teamcolor))
            pixbuf = self.create_teamcolor(team, red, green, blue)
        ...
        if ai_name != None:
            ...
            ## Adding Bot to Player List
                ...
                battle_status_bitmask = self.battlestatus_encode(ready, team, ally, mode, bonus, sync, side)
                battle_color_bitmask = self.color_encode(red, green, blue)
                args = ai_name + ' ' + str(battle_status_bitmask) + ' ' + str(battle_color_bitmask) + " " + (ai_location.replace(' ', "\t"))
                self.lobby.addbot(args, True)            

    def add_player(self, side):
    ## ==================================================================================================
    ...
    def update_army_value(self, widget, event, scan_iter, value):
        if self.online == False:
            self.player_liststore.set_value(scan_iter, 2, value)
        else:
            if (self.player_liststore.get_value(scan_iter, 10) != None) and ( (self.player_liststore.get_value(scan_iter, 11) == self.player_name_online) or (self.host == True) ):
                # Bot Name
                name = self.player_liststore.get_value(scan_iter, 1)
                # Getting Old BattleStatus & TeamColor
                ready, team, allyteam, mode, bonus, sync, side, red, green, blue = self.my_battlestatus(scan_iter)
                # Updating New Value
                team = value - 1
                allyteam = allyteam - 1
                # Encoding BattleStatus into Bitmasks
                battle_status_bitmask = self.battlestatus_encode(ready, team, allyteam, mode, bonus, sync, side)
                battle_color_bitmask = self.color_encode(red, green, blue)
                self.update_bot(name, battle_status_bitmask, battle_color_bitmask)
            elif self.player_liststore.get_value(scan_iter, 1) != self.player_name_online:
                self.force_teamnumber(scan_iter, value)
            else:
                self.update_my_battlestatus(scan_iter, ['team', value - 1])

    def update_ally_value(self, widget, event, scan_iter, value):
        if self.online == False:
            self.player_liststore.set_value(scan_iter, 8, value)
        else:
            if (self.player_liststore.get_value(scan_iter, 10) != None) and ( (self.player_liststore.get_value(scan_iter, 11) == self.player_name_online) or (self.host == True) ):
                # Bot Name
                name = self.player_liststore.get_value(scan_iter, 1)
                ...
                self.force_allynumber(scan_iter, value)
            else:
                self.update_my_battlestatus(scan_iter, ['ally', value - 1])

    def update_side_value(self, widget, event, scan_iter, value):
        if self.online == False:
            self.player_liststore.set_value(scan_iter, 7, self.sides[value])
        else:
            if (self.player_liststore.get_value(scan_iter, 10) != None) and ( (self.player_liststore.get_value(scan_iter, 11) == self.player_name_online) or (self.host == True) ):
                # Bot Name
                name = self.player_liststore.get_value(scan_iter, 1)
                ...
                self.update_my_battlestatus(scan_iter, ['side', value])

    def update_color_value(self, scan_iter, value):
        if self.online == False:
            self.update_color(scan_iter, value)
        else:
            if (self.player_liststore.get_value(scan_iter, 10) != None) and ( (self.player_liststore.get_value(scan_iter, 11) == self.player_name_online) or (self.host == True) ):
                # Bot Name
                name = self.player_liststore.get_value(scan_iter, 1)
                ...
                self.force_teamcolor(scan_iter, value)
            else:
                self.update_my_battlestatus(scan_iter, ['color', value[0], value[1], value[2]])

    def update_player_spectator_value(self, widget, event, scan_iter, value):

        if self.online == False:
            if value == 0:
                self.switch_to_spectator(scan_iter)
            else:
                self.switch_to_player(scan_iter)
        else:
            if self.player_liststore.get_value(scan_iter, 1) == self.player_name_online:
                self.update_my_battlestatus(scan_iter, ['mode', value])
            else:
                pass
                # TODO:

    def switch_to_player(self, scan_iter):
        ...
        self.player_liststore.set_value(scan_iter, 7, self.sides[0])        
        self.player_liststore.set_value(scan_iter, 8, ally)        
        self.player_liststore.set_value(scan_iter, 9, '0')        
        
        self.game_settings['Spectator Count'] = self.game_settings['Spectator Count'] - 1
        
        if (self.online == True) and (self.player_liststore.get_value(scan_iter, 1) == self.player_name_online):
            self.my_battlestatus(scan_iter)

    def switch_to_spectator(self, scan_iter):

        self.player_liststore.set_value(scan_iter, 2, None)
        self.player_liststore.set_value(scan_iter, 3, None)        
        self.player_liststore.set_value(scan_iter, 4, None)
        self.player_liststore.set_value(scan_iter, 5, None)        
        self.player_liststore.set_value(scan_iter, 6, None)        
        self.player_liststore.set_value(scan_iter, 7, None)        
        self.player_liststore.set_value(scan_iter, 8, None)        
        self.player_liststore.set_value(scan_iter, 9, None)
    ...
    def kick_player_or_bot(self, widget, event, scan_iter=None):
    ## ==================================================================================================
    ...
    def create_teamcolor(self, team, red, green, blue):
    ## ==================================================================================================
    ...
    ## ==================================================================================================
        ...
        pixbuf = self.create_teamcolor((team -1), red, green, blue)
        return red, green, blue, pixbuf

    def default_ai_name(self):
    ...
    def choose_color(self, widget, event, scan_iter):
    ## ==================================================================================================
    ##  Calls on Color Selector class.
    ##   And then passes updated rgb to self.update_color_value
    ## ==================================================================================================
        
        red = int(self.player_liststore.get_value(scan_iter, 3))
        green = int(self.player_liststore.get_value(scan_iter, 4))
        blue = int(self.player_liststore.get_value(scan_iter, 5))
        red = ((red * 65535) / 255)
        ...
        self.update_color_value(scan_iter, [red, green, blue])

    def update_color(self, scan_iter, value):
        team = str(self.player_liststore.get_value(scan_iter, 2))
        pixbuf = self.create_teamcolor(team, value[0], value[1], value[2])
    ...
    def get_teamcolor(self, team, ignore_iter=None):
    ## ==================================================================================================
    ##  Searchs self.player_liststore for First Team == team.
    ##   Returns Team red green blue pixbuf values
    ##    If scan_iter is passed, ignores if finds team with the passed scan_iter
    ## ==================================================================================================
    ...
    def treeview_team_image(self, column, cell, model, scan_iter):
    ## ==================================================================================================
    ##  Various functions to update images in Battle Player List.
    ## ==================================================================================================

        pixbuf = model.get_value(scan_iter, 6)
        cell.set_property('pixbuf', pixbuf)
        return

    def treeview_ready_image(self, column, cell, model, scan_iter):
    ## ==================================================================================================
    ##  Various functions to update images in Battle Player List.
    ## ==================================================================================================
        
        stock = model.get_value(scan_iter, 0)
        if stock != None:
    ...
    def treeview_sync_image(self, column, cell, model, scan_iter):
    ## ==================================================================================================
    ##  Various functions to update images in Battle Player List.
    ## ==================================================================================================
        
        stock = model.get_value(scan_iter, 13)
        if stock != None:
            pixbuf = self.player_treeview.render_icon(stock, gtk.ICON_SIZE_MENU, None)
        else:
            pixbuf = None
...
#---Update Player Name Code---#000000#FFFFFF------------------------------------
    ...
    ## ==================================================================================================
        ...
        if self.online == False:
            while scan_iter != None:
                if self.player_liststore.get_value(scan_iter, 1) == self.player_name_online:
                    self.player_liststore.set_value(scan_iter, 1, self.player_name_offline)
                elif self.player_liststore.get_value(scan_iter, 11) == self.player_name_online:
                    self.player_liststore.set_value(scan_iter, 11, self.player_name_offline)
                scan_iter = self.player_liststore.iter_next(scan_iter)              
        elif self.online == True:
            while scan_iter != None:
                if self.player_liststore.get_value(scan_iter, 1) == self.player_name_offline:
                    self.player_liststore.set_value(scan_iter, 1, self.player_name_online)
                elif self.player_liststore.get_value(scan_iter, 11) == self.player_name_offline:
                    self.player_liststore.set_value(scan_iter, 11, self.player_name_online)
                scan_iter = self.player_liststore.iter_next(scan_iter)

    def update_playername_offline(self):
    ## ==================================================================================================
    ...
    ## ==================================================================================================
        ...
        if self.online == False:
            while scan_iter != None:
                if self.player_liststore.get_value(scan_iter, 1) != None:
                    self.player_liststore.set_value(scan_iter, 1, self.player_name_offline)
                elif self.player_liststore.get_value(scan_iter, 11) != None:
                    self.player_liststore.set_value(scan_iter, 11, self.player_name_offline)
                scan_iter = self.player_liststore.iter_next(scan_iter)                    
    ...
    ## ==================================================================================================
        ...
        if self.online == True:
            ...
            if ready == False:
                self.update_my_battlestatus(None, ['ready', 0])
            else:
                self.update_my_battlestatus(None, ['ready', 1])                
        else:
            player_name = self.player_name_offline

        # Update Player Ready Icon
        scan_iter = self.scan_for_player(player_name)
        if ready == False:
            self.player_liststore.set_value(scan_iter, 0, gtk.STOCK_NO)
        else:
            self.player_liststore.set_value(scan_iter, 0, gtk.STOCK_YES)
        ...
        scan_iter = self.player_liststore.get_iter_first()
        while scan_iter != None:
            if self.player_liststore.get_value(scan_iter, 0) == gtk.STOCK_NO:
                break
            scan_iter = self.player_liststore.iter_next(scan_iter)
        if scan_iter == None:
            if self.host == True:
...
#---Update Map Code---#000000#FFFFFF-------------------------------------------- 
    def map_selection(self, map_selection):
    ## ==================================================================================================
    ...
    ## ==================================================================================================
        ...
        if paths:
            scan_iter = self.map_liststore.get_iter(paths[0])
            
            # No need to pass map_selection to self.update_battleinfo
            #  Faster & easier to update here & just pass None
            #   So it will be updated to other clients if online & host
            map_hashcode = self.map_liststore.get_value(scan_iter, 10)
            if self.game_settings['map hashcode'] != map_hashcode:
                map_name = self.map_liststore.get_value(scan_iter, 0)
                self.game_settings['map'] = map_name
                self.game_settings['map hashcode'] = map_hashcode

                self.update_battleinfo(None)
                self.update_my_syncstatus('map', 'Good')
                self.generate_map_preview(scan_iter)   
                            
    def search_for_map(self):
    ## ==================================================================================================
    ...
    ## ==================================================================================================
        ...
        scan_iter = self.map_liststore.get_iter_first()
        while scan_iter != None:
            if self.map_liststore.get_value(scan_iter, 0) == self.game_settings['map']:                   
                if self.map_liststore.get_value(scan_iter, 10) == self.game_settings['map hashcode']:
                    self.update_my_syncstatus('map', 'Good')
                    map_selection.select_iter(scan_iter)
                else:
                    self.update_my_syncstatus('map', 'Bad Checksum')
                break
            scan_iter = self.map_liststore.iter_next(scan_iter)
        
        if scan_iter == None:
            self.update_my_syncstatus('map', 'Missing')
        self.generate_map_preview(scan_iter)

    def generate_map_preview(self, scan_iter):
    ## ==================================================================================================
    ...
    ## ==================================================================================================
        if scan_iter == None:
            ...
            map_height = self.map_liststore.get_value(scan_iter, 2)
            map_width = self.map_liststore.get_value(scan_iter, 3)
...
#---Update Mod Code---#000000#FFFFFF--------------------------------------------
    def mod_selection(self, mod_selection):
    ## ==================================================================================================
    ...
    ## ==================================================================================================
        ...
        if paths:
            scan_iter = self.mod_liststore.get_iter(paths[0])
            
            # No need to pass map_selection to self.update_battleinfo
            #  Faster & easier to update here & just pass None
            #   So it will be updated to other clients if online & host
            mod_hashcode = self.mod_liststore.get_value(scan_iter, 1)
            if self.game_settings['mod hashcode'] != mod_hashcode:
                mod = self.mod_liststore.get_value(scan_iter, 0)
                self.game_settings['mod'] = mod
                self.game_settings['mod hashcode'] = mod_hashcode

                self.update_my_syncstatus('mod', 'Good')
                self.update_mod_sides(scan_iter)
                
    def search_for_mod(self):
    ## ==================================================================================================
    ...
    ## ==================================================================================================
        ...
        scan_iter = self.mod_liststore.get_iter_first()
        while scan_iter != None:
            if self.mod_liststore.get_value(scan_iter, 0) == self.game_settings['mod']:                   
                if self.mod_liststore.get_value(scan_iter, 1) == self.game_settings['mod hashcode']:
                    self.update_my_syncstatus('mod', 'Good')
                    mod_selection.select_iter(scan_iter)                    
                    self.update_mod_sides(scan_iter)
                else:
                    self.update_my_syncstatus('mod', 'Bad Checksum')
                    # TODO: Change Player Side to 0  just to be safe
                break
            scan_iter = self.mod_liststore.iter_next(scan_iter)
        
        if scan_iter == None:
            self.update_my_syncstatus('mod', 'Missing')
            # TODO: Change Player Side to 0  just to be safe

    def update_mod_sides(self, scan_iter):
    ...
    ##  If true updates sides
        ...
        scan_iter = self.player_liststore.get_iter_first()
        while scan_iter != None:
            old_team_side = (self.player_liststore.get_value(scan_iter, 7))
            for i in range(0, len(self.sides)):
                if old_team_side == self.sides[i]:
                    if (len(sides)) >= i:
                        self.player_liststore.set_value(scan_iter, 7, sides[i])
                    else:
                        self.player_liststore.set_value(scan_iter, 7, sides[0])
            scan_iter = self.player_liststore.iter_next(scan_iter)
        self.sides = sides                

#---Battle Online Code---#000000#FFFFFF-----------------------------------------
    def network_game(self, widget):
    ...
    def host_network_game(self):
        ...
        self.update_playername()                       
        self.options_senstive()
               
        treeselection = self.mod_treeview.get_selection()
        (model, scan_iter) = treeselection.get_selected()
        if scan_iter == None:
            scan_iter = self.mod_liststore.get_iter_first()
        self.game_settings['mod hashcode'] = self.mod_liststore.get_value(scan_iter, 1)
        self.game_settings['mod name'] = self.mod_liststore.get_value(scan_iter, 0)
    ...
    def host_network_game_sucess(self, battle_id):
    ...
    def join_network_game(self, command):
        ...
        self.description_entry.set_text(self.lobby.battle_liststore.get_value(scan_iter, 11))
        if self.lobby.battle_password == None:
            self.password_entry.set_text('')
        else:
            self.password_entry.set_text(self.lobby.battle_password)
        self.max_players_spinner.set_value(int((self.lobby.battle_liststore.get_value(scan_iter, 18).split('/')[1].split(' ')[0])))
    ...
    def scan_for_network_game(self):
        scan_iter = self.lobby.battle_liststore.get_iter_first()
        while scan_iter != None:
            if self.lobby.battle_liststore.get_value(scan_iter, 0) == self.lobby.battle_id:
                return scan_iter           
            scan_iter = self.lobby.battle_liststore.iter_next(scan_iter)

    def abort_network_game(self):
    ...
    def update_battleinfo(self, foo, local_changes=True):
    ...
    def update_battledetails(self, foo, local_changes=True):
    ...
    def force_teamnumber(self, scan_iter, value):   
        args = self.player_liststore.get_value(scan_iter, 1) + ' ' + str(value-1)
        self.lobby.forceteamno(args)        
        
    def force_allynumber(self, scan_iter, value):
        args = self.player_liststore.get_value(scan_iter, 1) + ' ' + str(value-1)
        self.lobby.forceallyno(args)
        
    def force_teamcolor(self, scan_iter, value):
        args = self.player_liststore.get_value(scan_iter, 1) + ' ' + str(self.color_encode(value[0], value[1], value[2]))
        self.lobby.forceteamcolor(args)

    def update_bot(self, name, battlestatus, teamcolor, send=True):
        if send == True:
    ...
    def add_start_rect(self, allyno, position):
...
#---MyBattleStatus---#000000#FFFFFF-----------------------------------------------
    def my_battlestatus(self, scan_iter=None):

        if scan_iter == None:
            scan_iter = self.scan_for_player(self.player_name_online)
        ...
        team = self.player_liststore.get_value(scan_iter, 2)            
        ...
        if team == None:
            ...
            team = int(team) - 1
            mode = 1
            allyteam = int(self.player_liststore.get_value(scan_iter, 8)) - 1
            bonus = int(self.player_liststore.get_value(scan_iter, 9))
            side =  self.sides.index(self.player_liststore.get_value(scan_iter, 7))
    ...
    def update_my_battlestatus(self, scan_iter, value):
        
        # Getting Old BattleStatus
        ready, team, ally, mode, bonus, sync, side, red, green, blue = self.my_battlestatus(scan_iter)
    ...
    def battlestatus_encode(self, ready, team, allyteam, mode, bonus, sync, side):
    ...
    def clientbattlestatus(self, command):
        username = command[0]
        ready, team, ally, mode, handicap, sync, side = self.battlestatus_decode(int(command[1]))
        red, green, blue = self.battlestatus_color_decode(int(command[2]))
        ...
        scan_iter = self.scan_for_player(command[0])
        
        if scan_iter == None:
            scan_iter =  self.player_liststore.append([ready, username, team, red, green, blue, pixbuf, side, ally, handicap, None, None, None, sync])
        else:
            self.player_liststore.set_value(scan_iter, 0, ready)
            self.player_liststore.set_value(scan_iter, 2, team)
            self.player_liststore.set_value(scan_iter, 3, red)
            self.player_liststore.set_value(scan_iter, 4, green)
            self.player_liststore.set_value(scan_iter, 5, blue)
            self.player_liststore.set_value(scan_iter, 6, pixbuf)
            self.player_liststore.set_value(scan_iter, 7, side)
            self.player_liststore.set_value(scan_iter, 8, ally)
            self.player_liststore.set_value(scan_iter, 9, handicap)
            self.player_liststore.set_value(scan_iter, 13, sync)
    ...
    def battlestatus_decode(self, status):
    ...
    def battlestatus_color_decode(self, color):
    ...
    def color_encode(self, red, green, blue):
    ...
    def script_color(self, DecodedColor):
    ...
    def get_sync_status(self, name=None):
        ## Status 0 = Unkown
        ## Status 1 = Good
        ## Status 2 = Bad
        ...
        if name == None:
            ...
            scan_iter = self.scan_for_player(name)
            sync = self.player_liststore.get_value(scan_iter, 13)
    ...
    def update_my_syncstatus(self, foo, status):
        ## Foo == 'map'  or 'mod'
        ...
        scan_iter = self.scan_for_player(player_name)
        if scan_iter != None:
            self.player_liststore.set_value(scan_iter, 13, sync_status_image)
...
#---Spring Script / Monitor Code---#000000#FFFFFF-------------------------------
    ...
    def get_team_ally_script_values(self):
        ...
        for o in range(0, len(self.battle_player_list)):
            ...
            if old_value == None:
                ...
                # Load Ally Old Value
                old_value = self.player_liststore.get_value(scan_iter, 8)
    ...
    ## ==================================================================================================
        ...
        self.spring_launcher = Timer(1, self.check_spring_pid, spring, script)
        self.spring_launcher.start()
        
    def check_spring_pid(self, spring, script):
    ## ==================================================================================================
