...
void ViewWindow::Select (float sx, float sy, int w, int h, bool box)
{
	...
	buffer = esTempAlloc <uint> (bufsize);
	selector_list = esTempAlloc <ViewSelector*> ( bufsize );
	...
	esTempFree (selector_list, bufsize);
	esTempFree (buffer,bufsize);
}
...
void Camera::GetMatrix(Matrix& vm)
{
	if (ctlmode == FPS)  {
        ...
        Matrix tmp;
		vm.translation(-pos);//initial translation of the model, pos is the center of rotation
		tmp.yrotate(yaw);
		vm *= tmp;
		tmp.xrotate(pitch);
		vm *= tmp;
...
inline void setvalue(fltk::Widget *w, bool v) {
	...
	if (v) i->set_flag(fltk::VALUE);
	else i->clear_flag(fltk::VALUE);
}
