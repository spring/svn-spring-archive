...
namespace Springie.SpringNamespace
{
  ...
  public class Spring : IDisposable
  {
    ...
    List<string> joinedPlayers = new List<string>(); // players who are in game
    List<string> lostPlayers = new List<string>(); // players who lost the game
    ...
    public event EventHandler<SpringLogEventArgs> GameOver; // game has ended, returns username of last loser and list of player who are still in game and didnt lose (=winners)
    ...
    private void ProcessLogLine(string l)
    {
      ...
      if (l.StartsWith("Player")) {
        ...
        string luser = "";
        if (lostPlayers.Count > 0) luser = lostPlayers[lostPlayers.Count - 1]; else luser = "";
    ...
    void springProcess_Exited(object sender, EventArgs e)
    {
      ...
      talker.Close();
      talker = null;
      if (SpringExited != null) SpringExited(this, new EventArgs());
    }
