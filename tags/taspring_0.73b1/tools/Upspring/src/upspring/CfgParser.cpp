...
//-------------------------------------------------------------------------
// CfgBuffer - serves as an input for the config value nodes
//-------------------------------------------------------------------------

void CfgBuffer::ShowLocation() const {
	logger.Trace (NL_Debug, "In %s on line %d:", filename, line);
}
...
bool CfgValue::SkipKeyword (CfgBuffer& buf, const char *kw)
{
	...
	if (kw[a]) {
		buf.ShowLocation();
		logger.Trace (NL_Debug, "Expecting keyword %s\n", kw);
		return false;
	}
...
bool CfgValue::ParseIdent (CfgBuffer& buf, string& name)
{
	if(isalnum (*buf))
	{
		...
		buf.ShowLocation(), logger.Trace (NL_Debug, "Expecting an identifier instead of '%c'\n", *buf);
		return false;
	}
}
