...
class gui_lobby:
	...
	# Called when client is disconnected. Reinitialises Variables that store data.
		self.server_agreement = ''
...
########################
	...
	# Creates GUI for channel
		...
		entry.show()
		entry.connect("activate", self.parse_output_channel, self.lobby_channel_names[last_index])
		table.attach(entry, 0,2,1,2, gtk.FILL, gtk.FILL,0,0)

		# Notebook Page
		self.lobby_notebook.append_page(vpane, label_box)
		if closeable == True:
			label_box.connect('event', self.on_channel_tab_click, self.lobby_channel_names[last_index])

		gtk.threads_leave()


	def parse_output_channel(self, entry, channel_name):
	# Parses User Input
	...
	def remove_channel_buffers(self, channel_name):
	...
	def on_channel_tab_click(self, widget, event, channel_name, ):
	# Checks if event on tab is right click
		...
		if  event.type == gtk.gdk.BUTTON_PRESS:
			if event.button == 3:
				...
				self.remove_channel_buffers(channel_name)
...
###########################################
	...
	def create_pm(self, pm_name):
	# Creates Data Stores for pms
		textbuffer = gtk.TextBuffer(table=None)
	...
	# Creates GUI for channel
		...
		entry = gtk.Entry(max=0)
		entry.show()
		entry.connect("activate", self.parse_output_pm, self.lobby_pm_names[last_index])
		table.attach(entry, 0,2,1,2, gtk.FILL, gtk.FILL,0,0)

		# Notebook Page
		self.lobby_notebook.append_page(vpane, label_box)
		label_box.connect('event', self.on_pm_tab_click, self.lobby_pm_names[last_index])

		gtk.threads_leave()


	def parse_output_pm(self, entry, pm_name):
	# Parses User Input
		temp_str =  entry.get_text()
		entry.set_text('')
		if temp_str != None:
	...
	def remove_pm_buffers(self, pm_name):
		for i in range(0,len(self.lobby_pm_names)):
			if self.lobby_pm_names[i] == pm_name:
				...
				self.lobby_pm_names.pop(i)
				self.lobby_pm_buffers.pop(i)
				self.lobby_pm_textview.pop(i)
				self.lobby_pm_users.pop(i)
				break


	def on_pm_tab_click(self, widget, event, pm_name):
	# Checks if event on tab is right click
		# If it is Leaves Channel & Deletes Data Stores for the channel
		if  event.type == gtk.gdk.BUTTON_PRESS:
			if event.button == 3:
				self.remove_pm_buffers(pm_name)



	def update_textbuffer_pm(self, pm_name, username, text, tag=None):
	# Updates TextBuffers for pm
		for i in range (0,len(self.lobby_pm_names)):
			if self.lobby_pm_names[i] == pm_name:
				gtk.threads_enter()
				...
				iter = self.lobby_pm_buffers[i].get_end_iter()
				self.lobby_pm_buffers[i].insert(iter, '<' + username + '> ')
				self.lobby_pm_buffers[i].insert(iter, text)

				if self.lobby_pm_buffers[i].get_line_count() > 50:
...
####################################
	...
	def player_popup_menu(self, treeview, event):
		gtk.threads_leave()
		...
		if iter != None:
			...
			if event.type == gtk.gdk._2BUTTON_PRESS:
				if event.button == 1:
					self.create_pm(liststore.get_value(iter, 0))
		gtk.threads_enter()
...
# Lobby Protocols
	...
	def saidprivate(self, command):
		...
		for i in range(0, len(self.lobby_pm_names)):
			if command[0] == self.lobby_pm_names[i]:
				...
				break
		if index == -1:
			self.create_pm(command[0])

		text = ''
		for i in range(1, len(command)):
			text = text + ' ' + command[i]
		self.update_textbuffer_pm(command[0], command[0], text, tag=None)
	...
	def sayprivate(self, command):
		...
		for i in range(1, len(command)):
			text = text + ' ' + command[i]
		self.update_textbuffer_pm(command[0], self.username, text, tag=None)
	...
	def forceleavechannel(self, command):
		...
		for i in range(2, len(command)):
			text = text + ' ' + command[i]
		self.remove_channel_buffers(self, command[0])
