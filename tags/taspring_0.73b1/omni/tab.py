    ...
    def __init__(self, parent, tabManager, name, tabTitle, channel=None):
        ...
        if channel != None:
            ...
            if self.channel.lan != True:
                ...
                images_names["rank0-small"]     = images.Add(wx.Bitmap(os.path.join("resource/lobby/", "rank0-small.png"), wx.BITMAP_TYPE_PNG))
                images_names["rank1-small"]     = images.Add(wx.Bitmap(os.path.join("resource/lobby/", "rank1-small.png"), wx.BITMAP_TYPE_PNG))
                images_names["rank2-small"]     = images.Add(wx.Bitmap(os.path.join("resource/lobby/", "rank2-small.png"), wx.BITMAP_TYPE_PNG))
                images_names["rank3-small"]     = images.Add(wx.Bitmap(os.path.join("resource/lobby/", "rank3-small.png"), wx.BITMAP_TYPE_PNG))
                images_names["rank4-small"]     = images.Add(wx.Bitmap(os.path.join("resource/lobby/", "rank4-small.png"), wx.BITMAP_TYPE_PNG))
                images_names["admin-small"]     = images.Add(wx.Bitmap(os.path.join("resource/lobby/", "admin-small.png"), wx.BITMAP_TYPE_PNG))
                ...
                list_users = {
                    ...
                    2:("rank_image",gui_lobby.gui_list.TYPE_IMAGE,"Rank",wx.LIST_FORMAT_LEFT,40),
                }
                self.tab_listPeople.init(list = self.channel.userList,object = user,mapping = list_users, name="users",virtual_style=False)
...
#  anything other than the channel object!
    ...
    def addUser(self, user, position, firstJoin=False):
        ...
        if user.rank_image == 0 or user.rank_image == 1:
            t = str(user.getRank())
            
        self.tab_listPeople.InsertImageItem(position, user.status_image)
        self.tab_listPeople.SetStringItem(position, 2,"", user.rank_image)
        
        if not firstJoin:
            self.tab_listPeople.Sort(0)
