...
# to be processed.  (Such as ADDUSER)
    ...
    def lookupCommand(self, command, retry=False):
        ...
        if command[0] == "PONG":
            ...
            self.client_messageQueue.put(command)
        elif command[0] == "ADDUSER" and len(command) == 5:
            self.client_messageQueue.put(command)
            ...
            self.client_messageQueue.put(command)
        elif command[0] == "BATTLEOPENED" and len(command) >= 10:
            self.client_messageQueue.put(command)
            ...
            self.client_messageQueue.put(command)
        elif command[0] == "CLIENTSTATUS" and len(command) == 3:
            self.client_messageQueue.put(command)
...
# gui_lobby.py
    ...
    def processCommand(self, command):
        ...
        if command[0] == "PONG":
            ...
            self.parent_window.onChannelTopic(tmp_list)
        ...
        #==================================================
            ...
            self.parent_window.onChannelListReceive(command[1:])
        elif command[0] == "CLIENTS":
            self.parent_window.onClientListReceive(command[1:])
        elif command[0] == "JOINED":
            self.parent_window.onNewUserJoin(command[1:])
        elif command[0] == "JOIN":
            self.parent_window.onChannelJoin(command[1])
        elif command[0] == "JOINFAILED":
