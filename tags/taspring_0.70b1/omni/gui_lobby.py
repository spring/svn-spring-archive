    ...
    def OnGetImage(self, item, col):
        ...
        if type == self.TYPE_IMAGE:            
            if self.images_names.has_key(getattr(self.list[item], fieldName)):
                return self.images_names[getattr(self.list[item], fieldName)]
            else:
                return -1                
        else:
            return -1
...
# Always read the documentation carefully before updating code from the py.gen folder!
	...
	# /end edits ===================================================================
        ...
        self.list_lobby_1_games = gui_list(self, 901, style=wx.LC_REPORT|wx.LC_SINGLE_SEL|wx.SUNKEN_BORDER|wx.LC_VIRTUAL)
...
# onChannelListRecieve() ===========================================================================
# ==================================================================================================
#  When the client recieves the list of channels
#  on the server.
#  info = [channel1, channel2, ...]
    def onChannelListReceive(self, info):
        try:
            ...
            if self.tabManager.getNetTabCount() == 0:
                if(not self.channel.IsShown()):                    
                    self.channel.Show()
                    self.channel.updateChannels(info)
            else:
                if channel != None:
                    self.channel.updateChannels(info)
                else:
            ...
            print(msg + "\n  in event onChannelListReceive()")
            
# /end onChannelListRecieve() ======================================================================
...
#  info = [channame, {clients}]
    def onClientListReceive(self, info):
        try:
            for x in info[1:]:
                if self.tabManager.tabFromName(info[0]).channel.addUser(x) == False:
...
#  info = BATTLE_ID + type + founder + IP + port + maxplayers + passworded + rank + mapname & {title} & {modname}
# ==================================================================================================
    def onBattle_Open(self, info):
        try:
            ...
            # TODO unknown field 
            self.server.addBattle(cmd_1[0], cmd_1[1], cmd_1[3], cmd_1[4], cmd_1[5], cmd_1[6], cmd_1[7], cmd_1[8], cmd_1[9], cmd_sentences[1], cmd_sentences[2])
        except Exception, msg:
...
# ==================================================================================================
    def __do_init(self):
        ...
        list_games = {
            0:("passworded_string",gui_list.TYPE_TEXT,"P",wx.LIST_FORMAT_LEFT,33),
        ...
        self.list_lobby_1_games.init(list = self.server.battleList,object = battle,mapping = list_games)
