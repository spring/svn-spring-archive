...
{
	//ASSERT(m_hThread==NULL && !(SOCKET)m_socket);
...
void CIrcSession::Disconnect(const char* lpszMessage)
{
	...
	int minutes = (ltime % 3600) / 60;
	int hours = ltime / 3600;

	m_socket.Send("QUIT :Played the game Spring for %d hours, %d minutes, %d seconds.\r\n", hours, minutes, seconds );
