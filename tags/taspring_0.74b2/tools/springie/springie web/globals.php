<?
if (!@mysql_connect("localhost", "iamacup_stats", "")) exit("FAILED 0");
if (!@mysql_select_db("iamacup_stats")) exit("FAILED 0");


	function updatePlayer($name, $ip) {
		if ($name != "") {
			$res = mysql_query ("SELECT id FROM Players WHERE name='$name'");
			if (mysql_num_rows($res) > 0) {
				$id = mysql_result($res,0,0);
				mysql_query("UPDATE Players SET lastSeen = ".time()." WHERE id = $id");
			} else {
				mysql_query("INSERT INTO Players (name, lastSeen) VALUES ('$name', ".time().")");
				$id = mysql_insert_id();
			}
			if ($ip!="") {
				if ($ip == 0) $ip = ip2long($_SERVER['REMOTE_ADDR']); else $ip = ip2long($ip);
