  ...
  public class VotePreset : AbstractPoll, IVotable
  {
    ...
    public bool Init(TasSayEventArgs e, string[] words)
    {
      ...
      if (AutoHost.FilterPresets(words, ah, out vals, out indexes) > 0) {
        presetId = indexes[0];
        presetName = vals[0];
        tas.Say(TasClient.SayPlace.Battle, "", "Do you want to apply preset " + presetName + "? !vote 1 = yes, !vote 2 = no", true);
        return true;
      } else {
    ...
    public void TimeEnd()
    {
      tas.Say(TasClient.SayPlace.Battle, "", "not enough votes for preset", true);
    }
    ...
    public bool Vote(TasSayEventArgs e, string[] words)
    {
      ...
      if (CheckEnd(out winVote)) {
        if (winVote == 1) {
          tas.Say(TasClient.SayPlace.Battle, "", "vote successful - appling preset " + presetName, true);
