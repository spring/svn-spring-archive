...
namespace creg
{
	...
	public:
		...
		void Serialize (unsigned int* x) { Serialize((int*)x); }
		void Serialize (short* x);
		void Serialize (unsigned short* x) { Serialize((short*)x); }

		void Serialize (char* x) { Serialize(&x,1); }
		void Serialize (unsigned char* x) { Serialize (&x,1); }
	};
