...
class gui_list(wx.ListCtrl,ListCtrlAutoWidthMixin):
    ...
    sort_order = False       # The order (True = ascending, False = descending)
    ...
    def init (self, list, object, mapping, name, virtual_style=True):
        self.list = list
        self.mapping = mapping
        self.virtual_style = virtual_style
        self.names = name
        self.default_listboxcolor = self.GetTextColour()
    ...
    def OnGetItemText(self, item, col):
        ...
        if type == self.TYPE_TEXT:
            ...
            tmp = str(getattr(self.list[item], fieldName))
            return tmp
        else:
            return ""
    ...
    def Sort(self, col=-1):   
        ...
        if fieldName == "name" and self.names == "battles":
            #update whether or not the battle is started or not
                
            self.list.sort(key=operator.attrgetter("host_ingame"),reverse= (not self.sort_order))
        else:
        ...
        if self.virtual_style:
            ...
            for row in range(0,len(self.list)):                
                for col in range(0,len(self.mapping)):
                    ...
                    if self.names == "battles" and self.list[row].battleStarted():
                        self.SetStringItem(row, col, str(self.OnGetItemText(row,col)), self.OnGetImage(row,col))
                        self.SetItemTextColour(row, wx.Colour(0,0,97))
                    else:
                        self.SetStringItem(row,col, str(self.OnGetItemText(row,col)), self.OnGetImage(row,col))
                        self.SetItemTextColour(row, self.default_listboxcolor)
    ...
    def onChannel_Receive(self, info):
        try:
            ...
            if self.tabManager.getNetTabCount() == 0:
                ...
                self.channel.channel_names.append(info[0])
...
# ==================================================================================================
    def onBattle_Open(self, info):
        try:
            ...
            if len(cmd_1) < 9 or cmd_sentences < 2:
            ...
            print(str(msg) + "\n  in event onBattle_Open() ID:" + cmd_1[0])
...
# ==================================================================================================
    def __do_init(self):
        ...
        self.list_lobby_1_games.init(list = self.server.battleList,object = battle,mapping = list_games, name="battles", virtual_style=False)
