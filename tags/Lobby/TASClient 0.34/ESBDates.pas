...
begin
	Result := False;
	for I := 1 to 12 do
		if AnsiCompareText (Month, ShortMonthNames [I]) = 0 then
		begin
			Result := True;
			Break
		end;
...
begin
	Result := False;
	for I := 1 to 12 do
		if AnsiCompareText (Month, LongMonthNames [I]) = 0 then
		begin
			Result := True;
			Break
		end;
...
begin
	Result := False;
	for I := 1 to 12 do
		if AnsiCompareText (DOW, ShortDayNames [I]) = 0 then
		begin
			Result := True;
			Break
		end;
...
begin
	Result := False;
	for I := 1 to 12 do
		if AnsiCompareText (DOW, LongDayNames [I]) = 0 then
		begin
			Result := True;
			Break
		end;
...
begin
	try
		...
		sTime := ReplaceChStr (sTime, ':', TimeSeparator); //Change to Locale Settings
...
begin
	Tmp := Sec;
	Result:= LInt2EStr (tmp div 3600) + TimeSeparator;
	Tmp := Tmp mod 3600;
	Result := Result + LInt2ZStr (Tmp div 60, 2) + TimeSeparator;
	Tmp := Tmp mod 60;
...
var
	...
	begin
		...
		begin
			...
			if (Length (InDate) > Posn + 2) and
				(Copy (InDate, Posn + 2, 1) <> DateSeparator) then
			begin
				Insert (DateSeparator, InDate, Posn + 2);
			end;
			...
			begin
				...
				if Copy (InDate, Posn - 1, 1) <> DateSeparator then
					Insert (DateSeparator, InDate, Posn);
			end;
			Month := MM;
		end;
	end;
	...
	begin
		...
		begin
			...
			if Copy (InDate, 1, 1) = DateSeparator then
				Break;
		...
		while Copy (InDate, 1, 1) = DateSeparator do
			Delete (InDate, 1, 1);
	end;
...
begin
	...
	try
		...
		// Allow ' ', '-' and '/' as valid alternatives for DateSeparator
		InDate := ReplaceChStr (InDate, ' ', DateSeparator);
		InDate := ReplaceChStr (InDate, '-', DateSeparator);
		InDate := ReplaceChStr (InDate, '/', DateSeparator);
		InDate := ReplaceChStr (InDate, '\', DateSeparator);
		...
		while Copy (InDate, 1, 1) = DateSeparator do
			Delete (InDate, 1, 1);
		while Copy (InDate, Length(InDate), 1) = DateSeparator do
