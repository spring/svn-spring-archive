...
begin
  ...
  MapExists := False;
  for i := 0 to BattleForm.MapListBox.Items.Count - 1 do if Copy(BattleForm.MapListBox.Items[i], 1, Length(BattleForm.MapListBox.Items[i]) - 4) = Copy(MapFilename, 1, Length(MapFilename) - 4) then
  begin
    MapExists := True;
    Break;
  end;
...
begin
  ShellExecute(MainForm.Handle, nil, PChar(AnsiString(TOnlineMap(OnlineMaps[Self.Tag]).PageLink)), '', '', SW_SHOW);
end;
