...
const
  VERSION_NUMBER = '0.24'; // Must be float value! (with a period as a decimal seperator)
  ...
  SUPPORTED_SERVER_VERSIONS: array[0..2] of string = ('0.22', '0.23', '0.24'); // client can connect ONLY to server with one of these version numbers
  EOL = #13#10;
  ...
  try // try..finally
    ...
    begin
      ...
      if not BattleForm.FigureOutBestPossibleTeamAllyAndColor(True, team, ally, color) then
        BattleForm.SetMyBattleStatus(0, False, 0, 0, 0, 0) // this probably shouldn't of happen though
      else
        BattleForm.SetMyBattleStatus(BattleForm.MySideButton.Tag, False, team, ally, 1, color);
