...
type
  ...
  TBattleForm = class(TForm)
    ...
    procedure TBXLink2Click(Sender: TObject);
    ...
    procedure MapListBoxDrawItem(Control: TWinControl; Index: Integer;
...
procedure TBattleForm.TBXLink1Click(Sender: TObject);
begin
  ShellExecute(MainForm.Handle, nil, MODS_PAGE_LINK, '', '', SW_SHOW);
end;

procedure TBattleForm.TBXLink2Click(Sender: TObject);
begin
  OnlineMapsForm.Show;
end;
...
begin
  // check if user has a map before readying up:
  if (not BattleForm.AmIReady) and (BattleForm.MapListBox.Items.IndexOf(BattleState.Battle.Map) = -1) then
  begin
...
procedure TBattleForm.MapListBoxDrawItem(Control: TWinControl;
  Index: Integer; Rect: TRect; State: TOwnerDrawState);
begin
  // this ensures the correct highlite color is used
  (Control as TListBox).Canvas.FillRect(Rect);

  // hide the extension of the map file name (".smf"):
  (Control as TListBox).Canvas.TextOut(Rect.Left + 3, Rect.Top, Copy((Control as TListBox).Items[Index], 1, Length((Control as TListBox).Items[Index])-4));
end;
