{
  ...
  * modders should put side images within "SidePics" folder in archive file (sdz/sd7). Side images should
  ...
  - SpTBXLib 1.5 (http://club.telepolis.com/silverpointdev/sptbxlib/index.htm)
...
const
  VERSION_NUMBER = '0.26'; // Must be float value! (with a period as a decimal seperator)
  ...
  SUPPORTED_SERVER_VERSIONS: array[0..1] of string = ('0.25', '0.26'); // client can connect ONLY to server with one of these version numbers
  EOL = #13#10;
...
uses BattleFormUnit, PreferencesFormUnit, Misc, WaitForAckUnit,
  ...
  IgnoreListUnit, MuteListFormUnit;
...
begin
  ...
  begin
    ...
    BattleForm.MapListBox.ItemIndex := -1;
    Exit;
  end;
...
begin
  ...
  begin
    ...
    ColorImageList.AddMasked(bmp, clBlack);
...
//        MyTeamColorIndex := color;   -> not needed anymore, we should keep last used color anyway
    ...
    begin
      ...
      begin
        ...
        tmpInt := BattleForm.MapListBox.Items.IndexOf(Battle.Map);
        if tmpInt = -1 then
        begin
          if BattleState.Status = Hosting then
        ...
        begin
          ...
          begin
            TmpInt2 := BattleForm.MapListBox.ItemIndex;
            BattleForm.MapListBox.ItemIndex := tmpInt;
            if tmpInt <> TmpInt2 then
              BattleForm.MapListBox.OnClick(nil); // we MUST send nil as parameter, since the event checks if Sender is nil and that way he knows whether user clicked on map list or not
          end;
        end;
      end;
...
begin
  ...
  begin // user is away
    ...
    begin
      ...
      MainForm.TryToSendData('MYSTATUS ' + IntToStr(Status.Me.Status));
    end;
  end;
...
begin
  ...
  begin // do the main initialization
    ...
    // set ThemeType-s to all forms (we must do this here, after inizialization,
    // since doing it any earlier would mean we wouldn't apply it to all forms
    // since not all forms would be created yet at any earlier point) and apply
    // selected theme:
    PreferencesForm.SpTBXThemesComboBox.ItemIndex := 0;
    PreferencesForm.SpTBXRadioButton3.Checked := True;
    PreferencesForm.SpTBXThemesComboBox.OnChange(PreferencesForm.SpTBXThemesComboBox);
    ...
    case Column of
      ...
      begin
        if Preferences.MarkUnknownMaps then
          if BattleForm.MapListBox.Items.IndexOf(Battle.Map) = -1 then
...
begin
  ...
  begin
    ...
    TryToSendData('MYSTATUS ' + IntToStr(Status.Me.Status));
  end;
end;
