	...
	for (moreFiles = (int)handle; moreFiles != -1; moreFiles = _findnext(handle, &files)) {
		...
		if ((files.attrib & 16) != 0) {
			...
			if (files.name[0] != '.') {
				...
				Scan(newPath, checksum);
			}
		}

		// Or maybe an archive we should look into?
		else if (CArchiveFactory::IsArchive(files.name)) {

			// Determine whether to rely on the cached info or not
			bool cached = false;
			string lcname = files.name;
			transform(lcname.begin(), lcname.end(), lcname.begin(), (int (*)(int))tolower);
			string fullName = curPath + "/" + files.name;
			...
			if (aii != archiveInfo.end()) {
				...
				struct _stat si;
				if (_stat(fullName.c_str(), &si) == 0) {
					if (si.st_mtime == aii->second.modified) {
						cached = true;
			...
			if (!cached) {
				...
				if (ar) {
					...
					ai.path = curPath;
					struct _stat si;
					_stat(fullName.c_str(), &si);
					ai.modified = si.st_mtime;
					ai.origName = files.name;
					ai.updated = true;
