...
//	gs->players[0]->active=true;
	...
	hideInterface=false;
	gameOver=false;
	showClock=!!regHandler.GetInt("ShowClock",1);
	showPlayerInfo=!!regHandler.GetInt("ShowPlayerInfo",1);
	gamePausable=true;
...
void CGame::HandleChatMsg(std::string s,int player)
{
	...
	if(s.find(".take")==0){
		...
		for(int a=0;a<gs->activeTeams;++a){
			if(gs->allies[gs->team2allyteam[a]][gs->team2allyteam[sendTeam]]){
	...
	if(s.find(".setmaxspeed")==0 && (player==0 || (gameSetup && player-gameSetup->numDemoPlayers==0))){
		maxUserSpeed=atof(s.substr(12).c_str());
		if(gs->userSpeedFactor>maxUserSpeed)
			gs->userSpeedFactor=maxUserSpeed;
	}
	if(s.find(".setminspeed")==0 && (player==0 || (gameSetup && player-gameSetup->numDemoPlayers==0))){
