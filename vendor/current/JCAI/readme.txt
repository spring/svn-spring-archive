...
.groupstate     : shows the state of the current force groups
...
- Capturing an AI builder will result in a crash, because the AI can currently not detect that one of it's units is captured.
    Likewise, sharing a unit with the AI makes it crash as well.
- Ground force groups like to chase aircraft.
- It doesn't support water maps
- It doesn't build support units (defense/radar)
- Only spring style metal density maps work good atm,
     OTA style metal density maps especially the ones with low extractor radius perform badly.

Todo list
---------

- Update/Modify metal spot class
- Initial builders, predefined builders that will directly be build regardless of current resource reserves.
