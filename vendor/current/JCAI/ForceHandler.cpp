...
aiTask* ForceHandler::GetNewTask ()
{
	int best=-1;
	int bestwait;
	...
	if (best >= 0)
	{
		aiTask*nt = new aiTask(build[best].newDef, build[best].batch);
		build[best].newDef=0;
...
void ForceHandler::ShowGroupStates ()
{
	for (int a=0;a<batches.size();a++)  {
		...
		ChatMsgPrintf (ai->cb, "Group %s is in state: %s on(%d,%d)", gr->GetName(), groupstatestr[(int)gr->state], gr->current.x,gr->current.y);
	}
}
	...
	if (forceinfo)
	{
		...
		for (list<CfgListElem>::iterator gr = forceinfo->childs.begin(); gr != forceinfo->childs.end(); ++gr)
		{
			if (dynamic_cast <CfgList*> (gr->value))
				ParseForceGroup ( (CfgList *)gr->value, tbl, gr->name );
		}
		return true;
	}
