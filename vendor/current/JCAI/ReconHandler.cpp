...
aiTask* ReconHandler::GetNewTask ()
{
	int counts[50];
	int a,total=0;
	...
	if (!config.scouts || config.scouts->builds.empty () || units.size () >= config.maxForce ||
		config.minEnergy > bhd->averageProd.energy || config.minMetal > bhd->averageProd.metal)
		return 0;
	...
	return new aiTask (buildTable.GetDef (config.scouts->builds [a]->def), this);
}
