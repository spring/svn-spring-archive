...
GUIfont::GUIfont(const std::string& fontFilename,int fontsize)
{
	...
	for (unsigned int c=0; c<256; c++) 
	{
		...
		charWidth[c]=glyph->advance.x / (float)0x10000;
		...
		// vertex coords
		float vx1=glyph_bitmap->left;
		float vy1=-glyph_bitmap->top;
		float vx2=glyph_bitmap->left+width;
		float vy2=height-glyph_bitmap->top;
