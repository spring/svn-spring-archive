...
CProjectileHandler::CProjectileHandler()
{
	...
	for(int y=0;y<256;y++){//random dots
		for(int x=0;x<256;x++){
			tex[y+256][x][0]=205+gu->usRandFloat()*50;
			tex[y+256][x][1]=205+gu->usRandFloat()*50;
			tex[y+256][x][2]=205+gu->usRandFloat()*50;
			tex[y+256][x][3]=0;
	...
	for(int a=0;a<20;++a){//random dots
		int bx=gu->usRandFloat()*228;
		int by=gu->usRandFloat()*228+256;
		for(int y=0;y<16;y++){
			for(int x=0;x<16;x++){
			...
			for(vector<CUnit*>::iterator ui(units.begin());ui!=units.end();++ui){
				...
				if(dif.SqLength() < totalRadius*totalRadius){
					if(unit->isMarkedOnBlockingMap && unit->physicalState != CSolidObject::Flying){
						...
						int square=max(0,min(gs->mapSquares-1,closePos.x*(1./8.)+int(closePos.z*(1./8.))*gs->mapx));
