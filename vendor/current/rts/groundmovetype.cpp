		...
		for(int xt=0;xt<11;++xt){
			...
			if(floor(start.x)==floor(to.x)){
				...
				while(keepgoing){
					...
					lineTable[yt][xt].push_back(int2(floor(xp),floor(zp)));
				}
...
bool CGroundMoveType::CheckGoalFeasability(void)
{
	...
	int minx=max(0,(goal.x-goalDist)/(SQUARE_SIZE*2));
	int minz=max(0,(goal.z-goalDist)/(SQUARE_SIZE*2));

	int maxx=min(gs->hmapx-1,(goal.x+goalDist)/(SQUARE_SIZE*2));
	int maxz=min(gs->hmapy-1,(goal.z+goalDist)/(SQUARE_SIZE*2));
