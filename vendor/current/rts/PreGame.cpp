...
bool CPreGame::Update(void)
{
	...
	if(allReady){
		...
		stupidGlobalModName = "xta_se_v065.sdz";
		if (gameSetup)
...
void CPreGame::ShowMapList(void)
{
	...
	std::vector<string> files=CFileHandler::FindFiles("maps\\*.smf");
	std::vector<string> arFiles = archiveScanner->GetMaps();

	if(files.empty() && arFiles.empty()){
		MessageBox(0,"Couldnt find any map files","PreGame error",0);
		return;
	}
	for(std::vector<string>::iterator fi=files.begin();fi!=files.end();++fi){
		list->AddItem(fi->substr(fi->find_last_of('\\')+1).c_str(),fi->substr(fi->find_last_of('\\')+1).c_str());
	}
	
	for(std::vector<string>::iterator fi=arFiles.begin();fi!=arFiles.end();++fi){
		list->AddItem((*fi).c_str(), (*fi).c_str());
	}
	showList=list;
}
