...
void ForceConfig::Precalc (BuildTable *tbl)
{
...
UnitGroup::UnitGroup (MainAI *AI) : aiHandler(AI)
{
	goal.x=-1.0f;
...
aiUnit* UnitGroup::CreateUnit (int id)
{
...
void UnitGroup::SetMoving ()
{
	...
	if (goal.x >= 0)
	{
		...
		Command c;
		c.id = CMD_MOVE;
		c.params.push_back (ai->map.gblocksize * (goal.x + 0.5f));
		c.params.push_back (0.0f);
		c.params.push_back (ai->map.gblocksize * (goal.y + 0.5f));
...
{
	GameInfo *gi = ai->map.GetGameInfo (current);
...
{
	int2 s = ai->map.GetGameInfoCoords (float3(mid.x,0.0f,mid.y));
...
void UnitGroup::Update ()
{
	...
	if (current.x < 0)
		current = ai->map.GetGameInfoCoords (float3 (mid.x,0.0f,mid.y));
	...
	GameInfo *b = ai->map.GetGameInfo (goal);
	...
	GameInfo *cur = 0;
	cur = ai->map.GetGameInfo (current);
	...
	case ugroup_Pruning: {
		...
		if (find (cur->enemies.begin(),cur->enemies.end(), curTarget) == cur->enemies.end ())
		{
			...
			else if (lastCmdFrame < ai->cb->GetCurrentFrame () - 30)
				SetPruning ();
		}
		break;}
...
/*	for (int a=0;a<units.size();a++) {
		...
		ai->cb->GiveOrder (units[a]->id, c);
	}*/

	for (ptrvec<ForceUnit>::iterator i = units.begin();i != units.end(); ++i)
		ai->cb->GiveOrder ( i->id, c );

	lastCmdFrame = ai->cb->GetCurrentFrame();
}

void UnitGroup::SetNewGoal ()
{
	if (group->task == ft_Raid)
	{
		// pick a low threat sector
		goal = ai->map.FindRaidSector (ai->cb,ai->map.GetGameInfoCoords (float3(mid.x,0.0f,mid.y)));
	}
	else if (group->task == ft_AttackDefend)
	{
		goal = ai->map.FindAttackDefendSector (ai->cb,ai->map.GetGameInfoCoords (float3(mid.x,0.0f,mid.y)));
	}
...
bool UnitGroup::CalcPositioning (float2& pmin, float2& pmax, float2& mid, int2& cs)
{
	...
	for (ptrvec<ForceUnit>::iterator i=units.begin();i!=units.end();++i)
	{
		float3 p = ai->cb->GetUnitPos ( i->id );
		...
		int2 cur = ai->map.GetGameInfoCoords(p), dif;
...
ForceHandler::ForceHandler (MainAI *AI) : TaskHandler(AI)
{
	Init ();
}
...
ForceHandler::~ForceHandler()
{
	for (int a=0;a<batches.size();a++) 
		ai->RemoveHandler (batches[a]);
}

void ForceHandler::Init ()
{
	if (!config.Load (ai->sidecfg, &buildTable))
		throw "Failed to load force info";

	config.Precalc (&buildTable);
...
void ForceHandler::UpdateNextCategoryTask (int c)
{
	...
	if (state.newUnitTypeIndex >= 0) {
		...
		state.waitFrame = ai->cb->GetCurrentFrame ();
	}
}

BuildTask* ForceHandler::GetNewTask ()
{
	int best=-1;
	int bestwait;
	...
	for (int a=0;a<build.size();a++)
	{
		...
		int d = ai->cb->GetCurrentFrame () - build[a].waitFrame;
	...
	if (best >= 0)
	{
		BuildTask*nt = new BuildTask(build[best].newDef, build[best].batch);
		build[best].newDef=0;
...
{
	float metal = ai->cb->GetMetalIncome ();
	float energy = ai->cb->GetEnergyIncome ();
	...
	for (int a=0;a<build.size();a++)
	{
		...
		if (!s->batch)
		{
			...
			if (best >= 0)
			{
				UnitGroup *gr = s->batch = new UnitGroup (ai);
				gr->group = fgl [best];
				...
				s->waitFrame = ai->cb->GetCurrentFrame ();
				s->newDef = 0;
			}
		}
		else
...
void ForceHandler::ShowGroupStates ()
{
	for (int a=0;a<batches.size();a++)  {
		...
		logPrintf ("Group %s is in state: %s on sector %d,%d\n", gr->GetName(), groupstatestr[(int)gr->state], gr->current.x,gr->current.y);
	}
}
...
bool ForceConfig::ParseForceGroup (CfgList *cfg, BuildTable *tbl, const string& name)
{	
	...
	for (int a=0;forcetasktbl [a].name; a++)
		if (!stricmp (forcetasktbl [a].name, str)) 
		{
...
bool ForceConfig::Load (CfgList *cfg, BuildTable *tbl)
{
	...
	if (forceinfo)
	{
		...
		for (list<CfgListElem>::iterator gr = forceinfo->childs.begin(); gr != forceinfo->childs.end(); ++gr)
		{
			...
			{
				if (!ParseForceGroup ( (CfgList *)gr->value, tbl, gr->name ))
					return false;
			}
		}
		return true;
