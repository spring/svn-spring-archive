...
public class Battle {
	...
	public Client founder; // founder or host of the battle
	...
	public static Battle createBattleFromString(String s, Client founder) {
		...
		String[] parsed2 = Misc.makeSentence(parsed, 14).split("\t");
		if (parsed2.length != 3) return null;
		
		String pass = parsed[2];
		...
		try {
			...
			// parsed[2] is password
			port = Integer.parseInt(parsed[3]);
			maxPlayers = Integer.parseInt(parsed[4]);
			startMetal = Integer.parseInt(parsed[5]);
			startEnergy = Integer.parseInt(parsed[6]);
			maxUnits = Integer.parseInt(parsed[7]);
			startPos = Integer.parseInt(parsed[8]);
			gameEndCondition = Integer.parseInt(parsed[9]);
			limitDGun = Misc.strToBool(parsed[10]);
			diminishingMMs = Misc.strToBool(parsed[11]);
			hash = Integer.parseInt(parsed[12]);
			rank = Integer.parseInt(parsed[13]);
		...
		return new Battle(type, founder, pass, port, maxPlayers, startMetal, startEnergy, maxUnits, startPos, gameEndCondition, limitDGun, diminishingMMs, hash, rank, parsed2[0], parsed2[1], parsed2[2]);
	}
	
	public Battle(int type, Client founder, String password, int port, int maxPlayers, int startMetal, int startEnergy, int maxUnits, int startPos, int gameEndCondition, boolean limitDGun, boolean diminishingMMs, int hashCode, int rank, String mapName, String title, String modName) {
		this.ID = IDCounter++;
		this.type = type;
	...
	public String createBattleOpenedCommand() {
		return "BATTLEOPENED " + ID + " " + type + " " + founder.account.user + " " + founder.IP + " "+ port + " " + maxPlayers + " " + Misc.boolToStr(restricted()) + " " + rank + " " + mapName + "\t" + title + "\t" + modName;
	}
