...
type
  ...
  TBattleForm = class(TForm)
    ...
    procedure AddTextToChat(Text: string; Color: TColor);
    ...
    procedure ChangeForceTeamColor(ColordIndex: Integer);
...
uses
  ...
  MinimapZoomedFormUnit, PreferencesFormUnit, ChooseColorFormUnit,
  DisableUnitsFormUnit, InitWaitFormUnit, AddBotUnit, UpdateBotUnit, Math,
  OnlineMapsUnit, ReplaysUnit, StrUtils;
...
procedure TBattleForm.AddTextToChat(Text: string; Color: TColor);
begin
  if Preferences.TimeStamps then Text := '[' + TimeToStr(Now) + '] ' + Text;
  Misc.AddTextToRichEdit(ChatRichEdit, Text, Color, True);
...
begin
  ...
  // side:
  Inc(Result, SideComboBox.ItemIndex);
...
begin
  ...
  MainForm.TryToSendData('UPDATEBATTLEDETAILS ' + IntToStr(MetalTracker.Value) + ' ' + IntToStr(EnergyTracker.Value) + ' ' + IntToStr(UnitsTracker.Value) + ' ' +IntToStr(StartPosRadioGroup.ItemIndex) + ' ' +IntToStr(GameEndRadioGroup.ItemIndex) + ' ' + IntToStr(BoolToInt(LimitDGunCheckBox.Checked)) + ' ' + IntToStr(BoolToInt(DiminishingMMsCheckBox.Checked)));
end;
...
begin
  ...
  tmp := ModList.Count;
  ReInitLib; // will automatically update mod list
  ...
  GetSideList(SideList);
  if SideList.Count > 2 then
  begin
    MainForm.AddMainLog('Warning: more than 2 sides detected in current mod, truncating ... (more sides will be supported with next release, sorry!)', Colors.Error);
    while SideList.Count > 2 do SideList.Delete(SideList.Count-1);
  end;
  ...
  SideComboBox.ItemIndex := Max(0, SideComboBox.Items.IndexOf(Side1));
  AddBotForm.BotSideComboBox.ItemIndex := Max(0, AddBotForm.BotSideComboBox.Items.IndexOf(Side2));
...
begin
  ...
  ReadyCheckBox.Checked := False; // status is automatically updated in OnClick event (which is triggered by changing Checked property, in case if it differs from previous value - which is the case here)
  ...
  if CreateProcess(nil, 'spring.exe script.txt', nil, nil, false, CREATE_DEFAULT_ERROR_MODE + NORMAL_PRIORITY_CLASS,
                   nil, nil, BattleState.Process.startinfo, BattleState.Process.proc_info) then
  begin
    AddTextToChat('Game launched', Colors.Info);
...
//  SetMyBattleStatus(SideComboBox.ItemIndex, False, 0, 0, 1, MyTeamColorIndex); *** we will update it once we receive REQUESTBATTLESTATUS
  ...
  AddTextToChat('Joined battle', Colors.Info);
...
//  SetMyBattleStatus(SideComboBox.ItemIndex, False, 0, 0, 1, MyTeamColorIndex); *** we will update it once we receive REQUESTBATTLESTATUS
  ...
  AddTextToChat('Joined battle replay', Colors.Info);
...
begin
  ...
  AddTextToChat('Battle opened', Colors.Info);
...
begin
  ...
  AddTextToChat('Battle replay opened', Colors.Info);
...
begin
  if IsBattleActive then
  begin
    AddTextToChat('Battle closed', Colors.Info);
    PlayResSound('battle');
  end;
...
var
  ...
  limitdgun, diminishingmms: Boolean;
...
begin
  ...
  else Writeln(f, #9 + 'HostIP=' + BattleState.Battle.IP + ';');

  Writeln(f, #9 + 'HostPort=' + IntToStr(BattleState.Battle.Port) + ';');
  if Preferences.UseCustomUDPSourcePort then Writeln(f, #9 + 'SourcePort=' + IntToStr(Preferences.CustomUDPSourcePort) + ';');
  Writeln(f, '');
  Writeln(f, #9 + 'MyPlayerNum=' + IntToStr(BattleState.Battle.Clients.IndexOf(MainForm.GetClient(Status.Username))) + ';');
  Writeln(f, '');
//  Writeln(f, #9 + 'NumPlayers=' + IntToStr(BattleState.Battle.Clients.Count + BattleState.Battle.Bots.Count) + ';');
...
begin
  ...
  if BattleState.Status = Hosting then Script.ChangeHostIP('localhost') else Script.ChangeHostIP(BattleState.Battle.IP);
...
begin
  ...
  then
    ...
    begin
      AddTextToChat('Back from the game', Colors.Info);
...
procedure TBattleForm.ChangeForceTeamColor(ColordIndex: Integer);
begin
  with ForceTeamColorButton.Glyph.Canvas do
  begin
    Pen.Color := TeamColors[ColorIndex];
    Brush.Color := TeamColors[ColordIndex];
...
begin
  ...
  ChangeTeamColor(ChooseColorFormUnit.ColorIndex, True);
end;
...
begin
  ...
  AddTextToChat('Forcing ' + TClient(BattleState.Battle.Clients[ClientsListBox.ItemIndex]).Name + '''s team number ...', Colors.Info);
end;
...
begin
  ...
  AddTextToChat('Forcing ' + TClient(BattleState.Battle.Clients[ClientsListBox.ItemIndex]).Name + '''s ally number ...', Colors.Info);
end;
...
begin
  if not (ChooseColorForm.ShowModal = mrOK) then Exit;
  ChangeForceTeamColor(ChooseColorFormUnit.ColorIndex);
  MainForm.TryToSendData('FORCETEAMCOLOR ' + TClient(BattleState.Battle.Clients[ClientsListBox.ItemIndex]).Name + ' ' + IntToStr(ChooseColorFormUnit.ColorIndex));
end;

procedure TBattleForm.Label20Click(Sender: TObject);
begin
  ShellExecute(MainForm.Handle, nil, 'http://www.fileuniverse.com/?page=listing&ID=92', '', '', SW_SHOW);
end;
