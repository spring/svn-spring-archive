		...
		if(!(*tti)->tracks.empty()){
			...
			for(set<UnitTrackStruct*>::iterator ti=(*tti)->tracks.begin();ti!=(*tti)->tracks.end();){
				...
				if(camera->InView((track->parts.front().pos1+track->parts.back().pos1)*0.5,track->parts.front().pos1.distance(track->parts.back().pos1)+500)){
					...
					color2[3]=track->trackAlpha-(gs->frameNum-ppi->creationTime)*track->alphaFalloff;

					for(list<TrackPart>::iterator pi=++track->parts.begin();pi!=track->parts.end();++pi){
						color[3]=track->trackAlpha-(gs->frameNum-ppi->creationTime)*track->alphaFalloff;
						if(pi->connected){
	...
	for(std::list<Scar*>::iterator si=scars.begin();si!=scars.end();){
		...
		if(camera->InView(scar->pos,scar->radius+16)){
			...
			if(scar->creationTime+10>gs->frameNum)
				color[3]=scar->startAlpha*(gs->frameNum-scar->creationTime)*0.1;
			else
				color[3]=scar->startAlpha-(gs->frameNum-scar->creationTime)*scar->alphaFalloff;

			int sx=max(0,(pos.x-radius)/16.0f);
			int ex=min(gs->hmapx-1,(pos.x+radius)/16.0f);
			int sz=max(0,(pos.z-radius)/16.0f);
			int ez=min(gs->hmapy-1,(pos.z+radius)/16.0f);
			for(int x=sx;x<=ex;++x){
				for(int z=sz;z<=ez;++z){
					float px1=x*16;
					float pz1=z*16;
					...
					float tx1=min(0.5,(pos.x-px1)/(radius*4.0f)+0.25f);
					float tx2=max(0.0,(pos.x-px2)/(radius*4.0f)+0.25f);
					float tz1=min(0.5,(pos.z-pz1)/(radius*4.0f)+0.25f);
					float tz2=max(0.0,(pos.z-pz2)/(radius*4.0f)+0.25f);
...
void CGroundDecalHandler::UnitMoved(CUnit* unit)
{
	...
	if(!unit->myTrack){
		...
		ts->owner=unit;
		ts->lifeTime=30*decalLevel*unit->unitDef->trackStrength;
		ts->trackAlpha=unit->unitDef->trackStrength*25;
