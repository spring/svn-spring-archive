...
CR_BIND_DERIVED(CS3OModelType, IModelParser);

void S3OPiece::DrawStatic()
{
	glPushMatrix();
...
CS3OParser::~CS3OParser()
{
	...
	for(ui=units.begin();ui!=units.end();++ui){
		DeleteCS3OModel(ui->second->rootobjects3o);
		delete ui->second;
	}
}

void CS3OParser::DeleteCS3OModel(S3OPiece* o)
{
	for(std::vector<S3OPiece*>::iterator di=o->childs.begin();di!=o->childs.end();di++){
		DeleteCS3OModel(*di);
	}
	delete o;
}

C3DOModel* CS3OParser::Load3DO(string name,float scale,int side)
{
	...
	model->numobjects=0;
	S3OPiece* object=LoadPiece(fileBuf,header.rootPiece,model);
...
LocalS3DOModel* CS3OParser::CreateLocalModel(C3DOModel *model, vector<struct PieceInfo> *pieces)
{
...
void CS3OParser::CreateLocalModel(S3OPiece *model, LocalS3DOModel *lmodel, vector<struct PieceInfo> *pieces, int *piecenum)
{
	PUSH_CODE_MODE;
	ENTER_SYNCED;
...
S3OPiece* CS3OParser::LoadPiece(unsigned char* buf, int offset,C3DOModel* model)
{
	model->numobjects++;

	S3OPiece* piece=new S3OPiece;
	...
	for(int a=0;a<fp->numVertices;++a){
		piece->vertices.push_back(*(S3OVertex*)&buf[vertexPointer]);
...
void CS3OParser::FindMinMax(S3OPiece *object)
{
	std::vector<S3OPiece*>::iterator si;
	...
	std::vector<S3OVertex>::iterator vi;
...
void CS3OParser::DrawSub(S3OPiece* o)
{
	glVertexPointer(3,GL_FLOAT,sizeof(S3OVertex),&o->vertices[0].pos.x);
	glTexCoordPointer(2,GL_FLOAT,sizeof(S3OVertex),&o->vertices[0].textureX);
	glNormalPointer(GL_FLOAT,sizeof(S3OVertex),&o->vertices[0].normal.x);
...
void CS3OParser::CreateLists(S3OPiece *o)
{
	...
	for(std::vector<S3OPiece*>::iterator bs=o->childs.begin();bs!=o->childs.end();bs++){
		CreateLists(*bs);
	}
}
