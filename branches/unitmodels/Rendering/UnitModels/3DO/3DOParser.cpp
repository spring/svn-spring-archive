...
C3DOParser::~C3DOParser()
{
	map<string,C3DOModel*>::iterator ui;
...
void C3DOParser::DeleteS3DO(C3DOPiece *o)
{
	for(std::vector<C3DOPiece*>::iterator di=o->childs.begin();di!=o->childs.end();di++){
		DeleteS3DO(*di);
	}
	delete o;
}

IModel* C3DOParser::Load(string name, int team)
{
	...
	C3DOModel *model = new C3DOModel;
	C3DOPiece* object = new C3DOPiece;
	model->rootobj = object;
	object->isEmpty=true;
	model->name=name;
	model->numobjects=1;

	ReadChild(0, object, team, &numobjects);
...
void C3DOParser::GetVertexes(_3DObject* o,C3DOPiece* object)
{
...
void C3DOParser::GetPrimitives(C3DOPiece* obj,int pos,int num,vertex_vector* vv,int excludePrim,int side)
{
...
void C3DOParser::CalcNormals(C3DOPiece *o)
{
	for(std::vector<C3DOPrimitive>::iterator ps=o->prims.begin();ps!=o->prims.end();ps++){
		for(int a=0;a<ps->numVertex;++a){
			C3DOVertex* vertex=&o->vertices[ps->vertices[a]];
...
bool C3DOParser::ReadChild(int pos, C3DOPiece *root,int side, int *numobj)
{
	(*numobj)++;
...
void C3DOParser::CreateLists(C3DOPiece *o)
{
	...
	for(std::vector<C3DOPiece*>::iterator bs=o->childs.begin();bs!=o->childs.end();bs++){
		CreateLists(*bs);
	}
}
...
void C3DOParser::FindCenter(C3DOPiece *object)
{
	std::vector<C3DOPiece*>::iterator si;
	...
	std::vector<C3DOVertex>::iterator vi;
...
float C3DOParser::FindRadius(C3DOPiece *object,float3 offset)
{
	float maxSize=0;
...
float C3DOParser::FindHeight(C3DOPiece* object,float3 offset)
{
	float height=0;
	offset+=object->offset;
	...
	for(std::vector<C3DOVertex>::iterator vi=object->vertices.begin();vi!=object->vertices.end();++vi){
...
IModelInstance* C3DOParser::CreateModelInstance(IModel *model, int side)
{
	C3DOModel *mdl = dynamic_cast<C3DOModel*>(model);
	assert (mdl);

	C3DOModelInstance *inst = new C3DOModelInstance;
	inst->model = mdl;
...
LocalS3DOModel *C3DOParser::CreateLocalModel(C3DOModel *model, vector<struct PieceInfo> *pieces)
{
...
float3 LocalS3DOModel::GetPieceDirection(int piecenum)
{
	...
	if(pieces[p].original3do){
		C3DOPiece &orig = *pieces[p].original3do;
		...
		S3OPiece &orig = *pieces[p].originals3o;
		if (orig.vertices.size() < 2) {
			return float3(1,1,1);
		}
		else if (orig.vertices.size() > 2) {
