...
void CAirMoveType::Update(void)
{
	...
	if(reservedPad){
		...
		float3 relPos=unit->modelInst->GetPiecePos(reservedPad->piece);
...
void CAirMoveType::SlowUpdate(void)
{
	...
	if(owner->pos!=oldSlowUpdatePos){
		...
		qf->MovedUnit(owner);
		owner->isUnderWater=owner->pos.y+owner->model->GetHeight()<0;
	}
}
...
void CAirMoveType::UpdateAirPhysics(float rudder, float aileron, float elevator,float engine,const float3& engineVector)
{
	...
	if(gHeight>owner->pos.y-owner->model->GetRadius()*0.2 && !owner->crashing){
		...
		if(impactSpeed>0){
			...
			pos.y=gHeight+owner->model->GetRadius()*0.2+0.01;
