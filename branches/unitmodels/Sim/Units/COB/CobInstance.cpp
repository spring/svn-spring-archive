...
int CCobInstance::FindModelPiece(const std::string& name, IModelInstance *inst)
{
	unsigned int cur;

	int numModelPieces = inst->GetNumPieces();
	for(cur=0; cur<numModelPieces; cur++)
		if (inst->GetPiece(cur)->name == name) break;

	//Not found? Try again with partial matching
	if (cur == numModelPieces)
	{
		for (cur = 0; cur < numModelPieces; cur ++) {
			string pn = inst->GetPiece(cur)->name;
	...
	info->AddLine("FindModelPiece: Could not map script piece %s to model piece",name.c_str());
	return -1;
}
...
: script(script)
{
	...
	modelPiece2Script.resize(unit->modelInst->GetNumPieces());
	for (unsigned int i = 0; i < script.pieceNames.size(); ++i) {
		...
		pi.updated = false;
		pi.visible = true;
		pi.modelPieceIndex = FindModelPiece(pi.name, unit->modelInst);
		modelPiece2Script[pi.modelPieceIndex] = i;
...
bool CCobInstance::PieceExists(int piece)
{
	if (piece >= 0 && piece < pieces.size())
		return pieces[piece].modelPieceIndex >= 0;
...
void CCobInstance::EmitSfx(int type, int piece)
{
	...
	int mdlPiece = pieces[piece].modelPieceIndex;
	...
	float3 relPos = unit->modelInst->GetPiecePos(mdlPiece);
	...
	switch (type) {
		case 4:
		case 5:		{	//reverse wake
			float3 relDir = -unit->modelInst->GetPieceDirection(mdlPiece) * 0.2;
...
#ifndef _CONSOLE
	float3 pos = unit->modelInst->GetPiecePos(mdlPiece) + unit->pos;
...
/*
	LocalS3DO * pieceData = &( unit->modelInst->pieces[unit->modelInst->scritoa[piece]] );
	if (flags & 1) {		//Shatter
		ENTER_MIXED;
...
//		info->AddLine("Shattering %i %f",dl->prims.size(),pieceChance);
		...
		C3DOPiece* dl = pieceData->original3do;
		if(dl){
			// 3DO 

			for(std::vector<C3DOPrimitive>::iterator pi=dl->prims.begin();pi!=dl->prims.end();++pi){
		...
		S3OPiece* cookedPiece = pieceData->originals3o;
		if (cookedPiece){
			// S3O 
			...
			if (cookedPiece->primitiveType == 0){
				...
				for (int i = 0; i < cookedPiece->vertexDrawOrder.size(); i += 3){
					...
					S3OVertex * verts = new S3OVertex[4];
...
#ifndef _CONSOLE
	float3 relpos = unit->modelInst->GetPiecePos(pieces[piece].modelPieceIndex);
	float3 pos=unit->pos + unit->frontdir*relpos.z + unit->updir*relpos.y + unit->rightdir*relpos.x;
...
int CCobInstance::GetUnitVal(int val, int p1, int p2, int p3, int p4)
{
	...
	case PIECE_XZ:{
		...
		float3 relPos = unit->modelInst->GetPiecePos(pieces[p1].modelPieceIndex);
		float3 pos = unit->pos + unit->frontdir * relPos.z + unit->updir * relPos.y + unit->rightdir * relPos.x;
...
/// Calculate transformation matrices for all model pieces and copy it to the model instance
void CCobInstance::UpdateModelState()
{
	IModelInstance *inst = unit->modelInst;

	UpdatePieceTransform(0);
}

void CCobInstance::UpdatePieceTransform(int mdlPiece)
{
	IModelInstance *inst = unit->modelInst;
	ModelInstancePiece *mip = inst->GetPiece(mdlPiece);
	CMatrix44f mat;

	if(mip->parent >= 0)
		mat = inst->GetPiece(mip->parent)->transform;

	PieceInfo& info = pieces[modelPiece2Script[mdlPiece]];
//	mat.Translate(info.offset.x,info.offset.y,-info.offset.z);

	#define CORDDIV 65536.0f
	#define ANGDIV 182.0f

	mat.Translate(-info.coords[0]/CORDDIV, info.coords[1]/CORDDIV, -info.coords[2]/CORDDIV);
	if(info.rot[1])
		mat.RotateY(info.rot[1]*(PI/32768));
	if(info.rot[0])
		mat.RotateX(info.rot[0]*(PI/32768));
	if(info.rot[2])
		mat.RotateZ(info.rot[2]*(PI/32768));
