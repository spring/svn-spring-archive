...
CUnit::~CUnit()
{
	if(delayedWreckLevel>=0){
		featureHandler->CreateWreckage(pos,wreckName, heading, delayedWreckLevel,-1,true,unitDef->name, team);
	}
...
void CUnit::Draw()
{
	...
	if(beingBuilt){
		if(shadowHandler->inShadowPass){
			...
			float start=model->GetBBox().min.y;
...
void CUnit::ChangeTeam(int newteam,ChangeType type)
{
	...
	model = modelLoader->LoadModel(unitDef->model.modelpath, newteam);
	delete modelInst;
	modelInst = model->GetType()->CreateModelInstance(model, newteam);
...
{
	relMidPos=model->GetRelMidPos();

	midPos=pos+frontdir*relMidPos.z + updir*relMidPos.y + rightdir*relMidPos.x;
	losHeight=relMidPos.y+radius*0.5;
	height = model->GetHeight();		//TODO: This one would be much better to have either in Constructor or UnitLoader!//why this is always called in unitloader
	...
	if(pos.y+model->GetHeight()<1)	//some torp launchers etc is exactly in the surface and should be considered uw anyway
		isUnderWater=true;
...
void CUnit::FinishedBuilding(void)
{
	...
	if(unitDef->isFeature){
		UnBlock();
		CFeature* f=featureHandler->CreateWreckage(pos,wreckName, heading, 0,allyteam,false,"", team);
		if(f){
