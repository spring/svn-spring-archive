...
//		unit->mainDamageType=unit->weapons.front()->damageType;
	...
	unit->model = modelLoader->LoadModel(ud->model.modelpath, side);
	 //this is a hack to make aircrafts less likely to collide and get hit by nontracking weapons
	unit->SetRadius( (ud->canfly?0.5f:1.0f) * unit->model->GetRadius());
	...
	unit->modelInst = unit->model->GetType()->CreateModelInstance(unit->model, unit->team);
