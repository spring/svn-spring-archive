...
CFeature* CFeatureHandler::CreateWreckage(const float3& pos, const std::string& name, float rot, 
										   int iter,int allyteam,bool emitSmoke,std::string fromUnit, int fromTeam)
{
	ASSERT_SYNCED_MODE;
	if(name.empty())
		return 0;
	...
	if(iter>1){
		return CreateWreckage(pos,fd->deathFeature,rot,iter-1,allyteam,emitSmoke,"", fromTeam);
	} else {
		if(!fd->modelname.empty()){
			CFeature* f=new CFeature;
			f->Initialize (pos,fd,(short int)rot,allyteam,fromUnit, fromTeam);
...
void CFeatureHandler::LoadFeaturesFromMap(bool onlyCreateDefs)
{
	...
	if(!onlyCreateDefs){
		...
		for(int a=0;a<numFeatures;++a){
			...
			(new CFeature)->Initialize (float3(mfi[a].pos.x, ypos,mfi[a].pos.z),featureDefs[name],(short int)mfi[a].rotation,-1,"",0);
		}
		delete[] mfi;
	}
}
	...
	for(int a=0;a<MAX_FEATURES;++a){
		...
		if(exists){
			if(loading){
				...
				CFeature* f = new CFeature;
				f->Initialize (pos,featureDefs[def],rotation,-1,fromUnit, 0);
			} else {
...
{
	float3 interPos=feature->pos+UpVector*feature->def->model->GetHeight()*0.5f;
	...
	if(snurr>0xffff)
		snurr-=0xffff;
	snurr=snurr>>13;
	assert (feature->def->model->HasFarTexture());
