...
//	centerPos.y=(ground->GetHeight(centerPos.x,centerPos.z));
	...
	/*
	 * cross1 is the sundir crossed with the UpVector,
	 * yielding a vector pointing to the side when
	 * considering the sun's view
	 * Controls left/right of view
	 */
	...
	/*
	 * cross2 is cross1 crossed with the sundir, yielding
	 * a vector pointing upwards but tilted according to the
	 * sun's view.
	 * Controls up/down of view
	 */
	...
	/*
	 * Sundir is the sun's view vector
	 */
	...
	/*
	 * Store the old up direction
	 */
	float3 oldup=camera->up;

	/*
	 * Orient the camera the same way the sun is pointing
	 */
	camera->right=shadowHandler->cross1;
	camera->up=shadowHandler->cross2;

	/*
	 * Move the camera's orthodirection in the sun's direction
	 */
	...
	/*
	 * Restore old up and original orthodirection
	 */
	camera->up=oldup;
