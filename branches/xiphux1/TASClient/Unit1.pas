{
  ...
  * Use modified TjanTracker component and other components found under "3RD-PARTY COMPONENTS" to compile
...
uses
  ...
  JvBaseDlg, JvDesktopAlert, JvDesktopAlertForm;
...
const
  VERSION_NUMBER = '0.195'; // Must be float value! (with a period as a decimal seperator)
  ...
  SUPPORTED_SERVER_VERSIONS: array[0..1] of string = ('0.194', '0.195'); // client can connect ONLY to server with one of these version numbers
  EOL = #13#10;
...
uses BattleFormUnit, PreferencesFormUnit, Misc, WaitForAckUnit,
  ...
  HostBattleFormUnit, AgreementUnit, PerformFormUnit;
...
begin
  ...
  if (Msg.CharCode = 9) and (ActiveControl.ClassType = TEdit) and (ActiveControl.Name = 'InputEdit') then
  begin
    Handled := True;
    
    if PageControl1.ActivePage.Caption = LOCAL_TAB then
      TryToAutoCompleteClientName(TEdit(ActiveControl), AllClients)
    else if PageControl1.ActivePage.Caption[1] = '#' then // a channel tab
      TryToAutoCompleteClientName(TEdit(ActiveControl), (PageControl1.ActivePage as TMyTabSheet).Clients)
...
begin
  AddTextToChatWindow(PageControl1.Pages[0] as TMyTabSheet, Text + ' [#' + IntToStr(AmbiguousCommandID) + ']', Color);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      MainForm.TryToSendData('GETINGAMETIME ' + sl[1]);
    end
...
begin
  ...
  text := Edit.Text;
  if Edit.SelStart = 0 then s := ''
  else s := Copy(Edit.Text, 1, Edit.SelStart);

  for i := 0 to Clients.Count-1 do
    if Copy(TClient(Clients[i]).Name, 1, Length(s)) = s then
      if not (s = TClient(Clients[i]).Name) then
      begin
        Text := Copy(Text, Length(s)+1, Length(Text));
        Text := TClient(Clients[i]).Name + Text;
        pos := Length(TClient(Clients[i]).Name);
        found := True;
        Break;
      end
      else
      begin
        Text := Copy(Text, Length(s)+1, Length(Text));
        Text := TClient(Clients[(i+1) mod Clients.Count]).Name + Text;
        pos := Length(TClient(Clients[(i+1) mod Clients.Count]).Name);
        found := True;
        Break;
      end;
