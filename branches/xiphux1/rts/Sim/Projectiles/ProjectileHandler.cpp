...
void CProjectileHandler::Draw(bool drawReflection)
{
	...
	for(std::list<FlyingPiece*>::iterator pi=flyingPieces.begin();pi!=flyingPieces.end();++pi){
		...
		vertex3d* v=&(*pi)->object->vertices[(*pi)->prim->vertices[0]];
	...
	for(psi=ps.begin();psi != ps.end();++psi){
		if(cam2->InView((*psi)->pos,(*psi)->drawRadius) && (loshandler->InLos(*psi,gu->myAllyTeam) || gu->spectating || ((*psi)->owner && gs->Ally((*psi)->owner->allyteam,gu->myAllyTeam)))){
			...
			if((*psi)->s3domodel){
				if((*psi)->s3domodel->imodel_type == IMODEL_TYPE_S3O){
					unitDrawer->QueS3ODraw(*psi,(*psi)->s3domodel->imodel_type);
				} else {
					(*psi)->DrawUnitPart();
				}
			}
...
void CProjectileHandler::AddFlyingPiece(float3 pos,float3 speed,IModel* object,S3DOPrimitive* piece)
{
