...
//	if (flags & 4)
	...
	IModel* dl = unit->localmodel->pieces[unit->localmodel->scritoa[piece]].original;
	if(dl){
		if (flags & 1) {		//Shatter
			ENTER_MIXED;
	...
	//		info->AddLine("Shattering %i %f",dl->prims.size(),pieceChance);
			...
			for(std::vector<S3DOPrimitive>::iterator pi=((S3DO*)dl)->prims.begin();pi!=((S3DO*)dl)->prims.end();++pi){
