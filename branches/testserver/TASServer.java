...
public class TASServer {
	...
	public static boolean tryToExecCommand(String command, Client client) {
		...
		try {
			...
			else if (commands[0].equals("GETCHARSET")) {
				if (client.account.accessLevel() < Account.ADMIN_ACCESS) return false;
				if (commands.length != 1) return false;

				client.sendLine("SERVERMSG Current decoder charset: " + asciiDecoder.charset().toString());
				client.sendLine("SERVERMSG Current encoder charset: " + asciiEncoder.charset().toString());
			}
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].charAt(0) == '-') {
				...
				if (s.equals("PORT")) {
					...
					lanAdminPassword = args[i+2]; //Misc.encodePassword(args[i+2]);
