...
class Map_Index(Thread):
	...
	def __init__(self, map_index, map_index_file, map_count, progressbar):
		Thread.__init__(self)
		self.progressbar = progressbar
		self.map_index = map_index
		self.map_index_file = map_index_file
	...
	def run(self):
		...
		self.progressbar.set_text('Adding New Maps')
		for i in range(0,len(map_list)):
			self.progressbar.set_fraction((i+1) / float(self.map_count))
		...
		remove_checksums = []
		self.progressbar.set_text('Removing Missing Maps')
...
class index_map:
	...
	def __init__(self, map_index_file, progressbar):
		self.map_count = unitsync.GetMapCount()
		self.map_index_file = map_index_file

		self.progressbar = progressbar

		if os.path.isfile(map_index_file) == False:
			self.create_index()
	...
	def rescan_index(self):
		if self.map_count != 0:
			index_scan = Map_Index(self.map_index, self.map_index_file, self.map_count, self.progressbar)
			index_scan.start()
			index_scan.join()			
		else:
			create_index(self)
