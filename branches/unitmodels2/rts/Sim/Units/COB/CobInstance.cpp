...
bool CCobInstance::IsNodeVisible(int node)
{
	return pieces[node].visible;
}

void CCobInstance::GetNodeTransform(int node, CMatrix44f& transform)
{
	CobPieceInfo *pi = &pieces[node];
	
	#define INVCORDDIV (1.f/65536.0f)
	#define INVANGDIV (1.f/182.0f*PI/180.f)

	transform.Translate(pi->mpi->offset);
	transform.Translate(-pi->coords[0]*INVCORDDIV, pi->coords[1]*INVCORDDIV, pi->coords[2]*INVCORDDIV);
	if(pi->rot[1])
		transform.RotateY(-pi->rot[1]*INVANGDIV);
	if(pi->rot[0])
		transform.RotateX(-pi->rot[0]*INVANGDIV);
	if(pi->rot[2])
		transform.RotateZ(-pi->rot[2]*INVANGDIV);
...
void CCobInstance::GetAbsoluteNodeTransform(int node, CMatrix44f& transform)
{
	if (pieces[node].mpi->parent >= 0) {
		int parent = pieces[node].mpi->parent;
		GetAbsoluteNodeTransform(parent, transform);
	}

	GetNodeTransform(node, transform);
}

bool CCobInstance::IsBusy()
{
	return busy;
}

int CCobInstance::FindScriptPiece(const std::string& name)
{
	unsigned int cur;

	for(cur=0; cur<script.pieceNames.size(); cur++)
		if (script.pieceNames[cur] == name) break;

	//Not found? Try again with partial matching
	if (cur == script.pieceNames.size())
	{
		for (cur = 0; cur < script.pieceNames.size(); cur ++) {
			const std::string& pn = script.pieceNames[cur];
            int maxcompare = min(name.size(), pn.size());
			int j;
...
: script(script)
{
	CModel *mdl = unit->model;

	staticVars.resize((unsigned int)script.numStaticVars);
	for (unsigned int i = 0; i < script.numStaticVars; ++i) 
		staticVars[i] = 0;

	pieces.resize(mdl->GetNumPieces());
	scriptToModelIndex.resize(script.pieceNames.size());
	fill(scriptToModelIndex.begin(), scriptToModelIndex.end(), -1);

	for (unsigned int i = 0; i < pieces.size(); i ++) {
		CobPieceInfo& pi = pieces[i];
		pi.coords[0] = 0; pi.coords[1] = 0; pi.coords[2] = 0;
		pi.rot[0] = 0; pi.rot[1] = 0; pi.rot[2] = 0;
		pi.updated = false;
		pi.visible = true;
		pi.mpi = mdl->GetPieceInfo(i);
		pi.scriptIndex = FindScriptPiece(pi.mpi->name);
		if (pi.scriptIndex>=0)
			scriptToModelIndex[pi.scriptIndex] = i;
	}
...
int CCobInstance::Call(const string &fname, CBAnimFinished cb, void *p1, void *p2)
{
	vector<int> x;
	return Call(fname, x, cb, p1, p2);
}

int CCobInstance::Call(const string &fname, vector<int> &args, CBAnimFinished cb, void *p1, void *p2)
{
...
void CCobInstance::AnimAction(int id)
{
	vector<int> x;
	AnimAction(id, x, NULL, NULL, NULL);
}

void CCobInstance::AnimAction(int id, int p1)
{
	vector<int> x;
	x.push_back(p1);
	AnimAction(id, x, NULL, NULL, NULL);
...
void CCobInstance::AnimAction(int id, vector<int> &args)
{
	AnimAction(id, args, NULL, NULL, NULL);
}

void CCobInstance::AnimAction(int id, vector<int> &args, CBAnimFinished cb, void *p1, void *p2)
{
	...
	RealCall(fn, args, cb, p1, p2);
}

int CCobInstance::GetActionType(const char *name)
{
	return script.getFunctionId(name);
}
...
int CCobInstance::RealCall(int functionId, vector<int> &args, CBAnimFinished cb, void *p1, void *p2)
{
...
int CCobInstance::Tick(int deltaTime) 
{
	...
	while (it != anims.end()) {
		...
		CobPieceInfo& cpi = PieceByScriptIndex((*cur)->piece);
		cpi.updated = true;

		switch ((*cur)->type) {
			case AMove:
				done = MoveToward(cpi.coords[(*cur)->axis], (*cur)->dest, (*cur)->speed / (1000 / deltaTime));
				break;
			case ATurn:
				done = TurnToward(cpi.rot[(*cur)->axis], (*cur)->dest, (*cur)->speed / (1000 / deltaTime));
				break;
			case ASpin:
				done = DoSpin(cpi.rot[(*cur)->axis], (*cur)->dest, (*cur)->speed, (*cur)->accel, 1000 / deltaTime);
				break;
		}
...
void CCobInstance::AddAnim(AnimType type, int piece, int axis, int speed, int dest, int accel, bool interpolated)
{
	...
	if (!ai) {
		...
		if (piece >= NumScriptPieces()) {
			info->AddLine("Invalid piece in anim %d (%d)", piece, NumScriptPieces());
		}
	}
	ai->speed = speed;
	ai->dest = dest;
...
{
	CobPieceInfo& cpi = PieceByScriptIndex(piece);
	cpi.coords[axis] = destination;
	cpi.updated = true;
}
...
{
	CobPieceInfo& cpi = PieceByScriptIndex(piece);
	if (cpi.visible != visible) {
		cpi.visible = visible;
		cpi.updated = true;
	}
}

bool CCobInstance::PieceExists(int piece)
{
	return piece >= 0 && piece < NumScriptPieces();
}
...
void CCobInstance::EmitSfx(int type, int piece)
{
	...
	int modelPiece = scriptToModelIndex [piece]
	...
	float3 relPos = unit->localmodel->GetPiecePos(modelPiece);
	...
	switch (type) {
		case 4:
		case 5:		{	//reverse wake
			float3 relDir = -unit->localmodel->GetPieceDirection(modelPiece) * 0.2;
...
void CCobInstance::AttachUnit(int piece, int u)
{
	...
	if ((piece >= 0) && piece >= NumScriptPieces()) {
	...
	if(tu && uh->units[u]){
		//info->AddLine("attach");
		tu->AttachUnit(uh->units[u], scriptToModelIndex [piece]);
	}
#endif
}
...
#ifndef _CONSOLE
	float3 pos = unit->localmodel->GetPiecePos(modelPiece) + unit->pos;
...
//	if (flags & 4)
	...
	LocalS3DO * pieceData = &( unit->localmodel->pieces[modelPiece] );
	if (flags & 1) {		//Shatter
		ENTER_MIXED;
...
int CCobInstance::GetUnitVal(int val, int p1, int p2, int p3, int p4)
{
	...
	case PIECE_XZ:{
		...
		float3 relPos = unit->localmodel->GetPiecePos(scriptToModelIndex[p1]);
		float3 pos = unit->pos + unit->frontdir * relPos.z + unit->updir * relPos.y + unit->rightdir * relPos.x;
...
bool CCobInstance::SupportsAction(int id) 
{
...
void CCobInstance::MoveSmooth(int piece, int axis, int destination, int delta, int deltaTime)
{
	...
	int cur = PieceByScriptIndex(piece).coords[axis];
...
void CCobInstance::TurnSmooth(int piece, int axis, int destination, int delta, int deltaTime)
{
	...
	int cur = PieceByScriptIndex(piece).rot[axis];
