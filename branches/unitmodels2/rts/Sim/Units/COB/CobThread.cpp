...
void CCobThread::SetCallback(CBAnimFinished cb, void *p1, void *p2)
{
	callback = cb;
	cbParam1 = p1;
	cbParam2 = p2;
	...
	while (state == Run) {
		...
		switch(opcode) {
			...
			case SHOW:{
				r1 = GET_LONG_PC();
				int i;
				for (i = 0; i < ANIM_MaxWeapons; ++i)
					if (callStack.back().functionId == script.scriptIndex[ANIMFN_FirePrimary + i])
						break;

				// If true, we are in a Fire-script and should show a special flare effect
				if (i < ANIM_MaxWeapons) {
					owner->ShowFlare(r1);
				}
				else {
