...
//		info->AddLine("shield mult %f %f %i %i", 2-adir.dot(bonusShieldDir),bonusShieldSaved,id,attacker->id);
	...
	animator->AnimAction(ANIMFN_HitByWeapon, hitAngles);	
...
void CUnit::ChangeTeam(int newteam,ChangeType type)
{
	...
	delete localmodel;
	localmodel = modelParser->CreateLocalModel(model, animator);
...
void CUnit::UpdateTerrainType()
{
	if (curTerrainType != lastTerrainType) {
		animator->AnimAction(ANIMFN_SetSFXOccupy, curTerrainType);
...
void CUnit::CalculateTerrainType()
{
	...
	if (!animator->SupportsAction(ANIMFN_SetSFXOccupy)) 
		return;
...
void CUnit::FinishedBuilding(void)
{
	...
	if(unitDef->windGenerator>0)
	{
		if(wind.curStrength > unitDef->windGenerator)
		{
			animator->AnimAction(ANIMFN_SetSpeed, (int)(unitDef->windGenerator*3000.0f));
		}
		else
		{
			animator->AnimAction(ANIMFN_SetSpeed, (int)(wind.curStrength*3000.0f));
		}
		animator->AnimAction(ANIMFN_SetDirection, (int)GetHeadingFromVector(-wind.curDir.x, -wind.curDir.z));
	}
...
void CUnit::KillUnit(bool selfDestruct,bool reclaimed,CUnit *attacker)
{
	...
	if(!reclaimed && !beingBuilt){
		...
		args.push_back(0);
		animator->AnimAction(ANIMFN_Killed, args, &CUnitKilledCB, this, NULL);
...
void CUnit::Activate()
{
	...
	//	animator->AnimAction(ANIMFN_SetSpeed, (int)(readmap->tidalStrength*3000.0f*unitDef->tidalGenerator));
...
void CUnit::PushWind(float x, float z, float strength)
{
	...
	{
		animator->AnimAction(ANIMFN_SetSpeed, (int)(strength*3000.0f));
	}

	animator->AnimAction(ANIMFN_SetDirection, (int)GetHeadingFromVector(-x, -z));
}
