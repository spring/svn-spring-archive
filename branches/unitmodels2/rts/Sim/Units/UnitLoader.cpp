...
START_TIME_PROFILE;
	...
	CCobInstance *cob = new CCobInstance(GCobEngine.GetCobFile("scripts/" + name+".cob"), unit);
	unit->animator = cob;
	unit->localmodel = modelParser->CreateLocalModel(unit->model, unit->animator);
	...
	unit->animator->AnimAction(ANIMFN_Create);
	vector<int> args; 
	args.push_back(relMax);
	unit->animator->AnimAction("SetMaxReloadTime", args);
