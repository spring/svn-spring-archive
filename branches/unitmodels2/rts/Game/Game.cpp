...
END_TIME_PROFILE("Sim time")
	...
	for(int a=0;a<gs->activePlayers;++a){
		...
		std::vector<int> args;
		args.push_back(0);
		unit->animator->AnimAction(ANIMFN_AimFromPrimary/*/ANIMFN_QueryPrimary+weaponNum/**/,args);
...
void CGame::UpdateUI()
{
	...
	if(gu->directControl){
		...
		std::vector<int> args;
		args.push_back(0);
		owner->animator->AnimAction(ANIMFN_AimFromPrimary/*/ANIMFN_QueryPrimary+weaponNum/**/,args);
