...
public:
	C3DOParser();
	virtual ~C3DOParser();
	CModel* Load3DO(string name,float scale=1,int side=1);
...
private:
	...
	bool ReadChild(int pos,S3DO* root,int side, int *numobj,CModel *model, int parent);
	void DrawSub(S3DO* o);
	string GetLine(CFileHandler& fh);
	void CreateLocalModel(S3DO *model, CModelInstance *lmodel, int *piecenum);
	
	map<string,CModel*> units;
	set<string> teamtex;
