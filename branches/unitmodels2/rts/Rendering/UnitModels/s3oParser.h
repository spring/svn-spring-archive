...
private:
	SS3O* LoadPiece(unsigned char* buf, int offset,CModel* model, int parent);
	void DeleteSS3O(SS3O* o);
	void FindMinMax(SS3O *object);
	void DrawSub(SS3O* o);
	void CreateLists(SS3O *o);
	void CreateLocalModel(SS3O *model, CModelInstance *lmodel, int *piecenum);

	std::map<std::string,CModel*> units;
};
