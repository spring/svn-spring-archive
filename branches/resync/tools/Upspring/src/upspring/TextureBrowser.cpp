...
void TextureBrowser::Item::draw()
{
	...
	ilBindImage (tex->ilIdent);
	ILubyte *data = ilGetData ();
	ILint texWidth = ilGetInteger (IL_IMAGE_WIDTH);
	ILint texHeight = ilGetInteger (IL_IMAGE_HEIGHT);

	if(selected){
		fltk::Rectangle selr (-3,-3,texWidth+6,texHeight+6);
		fltk::push_clip(selr);
	...
	fltk::Rectangle rect (0,0,texWidth,texHeight);
	fltk::drawimage(data, fltk::RGBA, rect);
}
