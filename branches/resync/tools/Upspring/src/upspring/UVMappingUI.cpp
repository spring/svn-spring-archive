...
bool UVViewWindow::SetupChannelMask ()
{
	...
	if (!mdl->textures [textureIndex]) 
		return false;

	if (!hasTexCombine) {
		glEnable (GL_TEXTURE_2D);
		glBindTexture (GL_TEXTURE_2D, mdl->textures[textureIndex]->glIdent);
		return true;
	}
...
void UVViewWindow::DrawScene ()
{
	...
	for (int a=0;a<objs.size();a++) {
		...
		for (int p=0;p<obj->poly.size();p++) {
			Poly *pl=obj->poly[p];
			glBegin(GL_POLYGON);
			for (int v=0;v<pl->verts.size();v++) {
				Vertex &vrt = obj->verts [pl->verts[v]];
...
void MappingUI::flipUVs ()
{
	...
	for (int a=0;a<obj.size();a++)
	{
		...
		for (int b=0;b<o->verts.size();b++) {
			Vertex& vrt = o->verts[b];
			vrt.tc[0].y = 1.0f - vrt.tc[0].y;
		}
	}
}
