...
int C3O_IO::SaveObject (MdlObject *obj)
{
	...
	// write vertex chunk
	o.vertexTbl = SaveVertices (obj->verts);

	// write polygon chunk
	o.polyTbl = SavePolygons (obj->poly);
...
MdlObject* C3O_IO::LoadObject (int chunk)
{
	...
	obj->verts = LoadVertices (o.vertexTbl);
	obj->poly = LoadPolygons (o.polyTbl);
...
void C3O_IO::LoadTexture (Model *mdl, int chunk)
{
	...
	int index;
	if (type == C3OTEX_SPRING0)
		index = 0;
	else if (type == C3OTEX_SPRING1)
		index = 1;
	else
		return;

	mdl->textureNames [index] = ReadZStr (file);
}
...
bool Model::SaveC3O (const char *filename, IProgressCtl& progctl)
{
	...
	for (int a=0;a<2;a++) {
		if (!textureNames[a].empty())
			c3o.SaveTexture (a, textureNames[a]);
	}
...
bool Model::LoadC3O (const char *filename, IProgressCtl& progctl)
{
	...
	for (int a=0;a<2;a++) {
		if (!textureNames[a].empty()) {
			textures[a] = new Texture (textureNames[a], mdlPath);
			if (!textures[a]->IsLoaded ()) {
