...
//    (Name: 'Test/backup server (filehamster.net)'; Address: 'filehamster.net'),
    (Name: 'Backup server (filehamster.net)'; Address: 'filehamster.net'),
...
uses MainUnit, Misc, ShellAPI, Registry, BattleFormUnit, HostBattleFormUnit, Math,
  OnlineMapsUnit, NotificationsUnit, PerformFormUnit, HighlightingUnit,
  NewAccountUnit, IgnoreListUnit, MapListFormUnit;
...
begin
  try
    ...
    with BattleForm.VDTBattleClients.Header.Columns do
      for i := 0 to Count-1 do Misc.SetRegistryData(HKEY_CURRENT_USER, '\Software\TASClient\Forms\BattleForm', 'Column' + IntToStr(i), rdInteger, Items[i].Width);
