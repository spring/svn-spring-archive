...
const
  VERSION_NUMBER = '0.33'; // Must be float value! (with a period as a decimal seperator)
  ...
  SUPPORTED_SERVER_VERSIONS: array[0..1] of string = ('0.32', '0.33'); // client can connect ONLY to server with one of these version numbers
  EOL = #13#10;
  ...
  MODS_PAGE_LINK = 'http://www.unknown-files.net/index.php?page=browse&cat=1&subcat=2';
  MAPS_PAGE_LINK = 'http://www.unknown-files.net/index.php?page=browse&cat=1&subcat=4';
...
var
  ...
  Status: record
    ...
    LastCommandReceivedFull: string; // here we keep the full command we last received from the server (together with the param list)
...
//***      Misc.SendUDPStr('localhost', 8201, 0, 'Betalord');
    ...
    begin
      ...
      Pings.Add(pinginfo);
      TryToSendCommand('PING', IntToStr(key));
...
begin
  ...
  try // try..finally
    ...
    begin
      if (sl.Count <> 4) or (not CheckServerVersion(sl[1])) then
      begin
      ...
      if (sl[2] <> '*') and (sl[2] <> Status.MySpringVersion) then
      begin
...
begin;
  ...
  TryToSendCommand('LOGIN', Username + ' ' + Misc.GetMD5Hash(Password) + ' ' + IntToStr(Status.MyCPU) + ' ' + ip + ' ' + PROGRAM_VERSION);
end;
