...
namespace Springie.AutoHostNamespace
{
  public class AutoHostConfig
  {
    ...
    int defaultRightsLevelWithTempAdmin = 0;
    [Category("Rights")]
    [Description("Default rights level for non-privileged users when there is a tempadmin in game")]
    public int DefaultRightsLevelWithTempAdmin
    {
      get { return defaultRightsLevelWithTempAdmin; }
      set { defaultRightsLevelWithTempAdmin = value; }
    }
    ...
    public void AddMissingCommands()
    {
      ...
      AddMissing(new CommandConfig("ring", 0, "[<filters>..] - rings all unready or specific player(s), e.g. !ring - rings unready, !ring icho - rings Licho", 5, new TasSayEventArgs.Places[] { TasSayEventArgs.Places.Normal, TasSayEventArgs.Places.Battle, TasSayEventArgs.Places.Game }));
      ...
      AddMissing(new CommandConfig("say", 0, "<text> - says something in game", 0, new TasSayEventArgs.Places[] { TasSayEventArgs.Places.Normal, TasSayEventArgs.Places.Battle, TasSayEventArgs.Places.Game }));
      ...
      AddMissing(new CommandConfig("votekick", 0, "[<mapname>..] - starts vote to kick a player, e.g. !votekick Licho", 0, new TasSayEventArgs.Places[] { TasSayEventArgs.Places.Normal, TasSayEventArgs.Places.Battle, TasSayEventArgs.Places.Game }));

      AddMissing(new CommandConfig("voteforcestart", 0, " - starts vote to force game to start in lobby"));

      AddMissing(new CommandConfig("voteforce", 0, " - starts vote to force game to start from game", 0, new TasSayEventArgs.Places[] { TasSayEventArgs.Places.Normal, TasSayEventArgs.Places.Battle, TasSayEventArgs.Places.Game }));

      AddMissing(new CommandConfig("voteexit", 0, " - starts vote to exit game", 0, new TasSayEventArgs.Places[] { TasSayEventArgs.Places.Normal, TasSayEventArgs.Places.Battle, TasSayEventArgs.Places.Game }));
      ...
      AddMissing(new CommandConfig("voterehost", 0, "[<modname>..] - votes to rehost game, e.g. !rehost abosol 2.23 - rehosts AA2.23", 0, new TasSayEventArgs.Places[] { TasSayEventArgs.Places.Normal, TasSayEventArgs.Places.Battle, TasSayEventArgs.Places.Game }));
      ...
      AddMissing(new CommandConfig("maplink", 0, "[<mapname>..] - looks for maplinks at unknown-files", 5, new TasSayEventArgs.Places[] { TasSayEventArgs.Places.Battle, TasSayEventArgs.Places.Normal, TasSayEventArgs.Places.Channel }));

      AddMissing(new CommandConfig("modlink", 0, "[<modname>..] - looks for modlinks at unknown-files", 5, new TasSayEventArgs.Places[] { TasSayEventArgs.Places.Battle, TasSayEventArgs.Places.Normal, TasSayEventArgs.Places.Channel }));
      ...
      AddMissing(new CommandConfig("dlmod", 1, "[<modname/dllid>..] - downloads mod to server. You must specify mod file id (from unknown files)", 10));
      ...
      AddMissing(new CommandConfig("ally", 2, "<allynumber> [<playername>..] - forces given player to an alliance"));
      ...
      AddMissing(new CommandConfig("votepreset", 0, "[<presetname>..] - starts a vote to apply given preset"));

      AddMissing(new CommandConfig("presetdetails", 0, "[<presetname>..] - shows details of given preset", 2));

      AddMissing(new CommandConfig("cbalance", 1, "[<allycount>] - assigns people to allycount random balanced alliances but attempts to put clanmates to same teams", 10));
      ...
      AddMissing(new CommandConfig("smurfs", 0, "- finds smurfs, use this command to get more help", 5, new TasSayEventArgs.Places[] { TasSayEventArgs.Places.Normal, TasSayEventArgs.Places.Battle, TasSayEventArgs.Places.Channel }));

      AddMissing(new CommandConfig("stats", 0, "- displays statistics, use this command to get more help", 5, new TasSayEventArgs.Places[] { TasSayEventArgs.Places.Normal, TasSayEventArgs.Places.Battle, TasSayEventArgs.Places.Channel }));

      AddMissing(new CommandConfig("kickspec", 2, "[0|1] - enables or disables automatic spectator kicking", 0, new TasSayEventArgs.Places[] { TasSayEventArgs.Places.Battle, TasSayEventArgs.Places.Game, TasSayEventArgs.Places.Normal }));

      AddMissing(new CommandConfig("votekickspec", 0, "- starts a vote to enables or disable automatic spectator kicking", 0, new TasSayEventArgs.Places[] { TasSayEventArgs.Places.Battle, TasSayEventArgs.Places.Game }));

      AddMissing(new CommandConfig("setpassword", 4, "<newpassword> - sets server password (needs !rehost to apply)"));

      AddMissing(new CommandConfig("setminrank", 4, "<minrank> - sets server minimum rank (needs !rehost to apply)"));
      ...
      AddMissing(new CommandConfig("mincpuspeed", 4, "<GHz> - sets minimum CPU for this host - players with CPU speed below this value are auto-kicked, 0 = no limit"));


      AddMissing(new CommandConfig("tempadmin", 2, "<name> - sets <name> as a new temp admin, use without parameter to remove any current tempadmin. If there is temp admin on server, other non-admin people have their rights reduced"));

      AddMissing(new CommandConfig("votetempadmin", 0, "<name> - sets <name> as a new temp admin, use without parameter to remove any current tempadmin. If there is temp admin on server, other non-admin people have their rights reduced", 0, new TasSayEventArgs.Places[] { TasSayEventArgs.Places.Normal, TasSayEventArgs.Places.Battle, TasSayEventArgs.Places.Game }));

      AddMissing(new CommandConfig("autolock", 1, "<players> - sets desired number of players in game. If this number is reached, server will autolock itself, if someone leaves, it will autounlock again. !autolock without parameter disables auto locking"));
    }
