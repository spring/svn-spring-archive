...
bool SettingNode::SetProperty(std::string szProperty, std::string szValue) {
	...
	if (strcmpi(szProperty.c_str(),"Tag")==0) {
		m_szTag=szValue;
		return true;
	}
	return false;
...
SettingNode* SettingNode::FindTaggedChild(std::string tag) {
	for (std::vector<SettingNode*>::iterator i=m_children.begin();i!=m_children.end();++i) {
		if(!stricmp((*i)->m_szTag.c_str(), tag.c_str()))
			return *i;
		else {
			SettingNode *r = (*i)->FindTaggedChild(tag);
			if (r) return r;
		}
	}
	return 0;
