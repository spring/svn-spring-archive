...
//  Label7.Caption := 'Max. wind: ' + Format('%.5g', [MapInfo.MaxWind]);
  ...
  SetRadioItem(MapsPopupMenu, MapIndex);
end;
...
var
  i: Integer;
begin
  MapsPopupMenu.Items.Clear;
  for i := 0 to Utility.MapList.Count-1 do
  begin
    MapsPopupMenu.Items.Add(TSpTBXItem.Create(MapsPopupMenu.Items.Owner));

    with MapsPopupMenu.Items[MapsPopupMenu.Items.Count-1] as TSpTBXItem do
    begin
      Caption := Utility.MapList[i];
      Tag := i;
      RadioItem := True;
...
begin
  ...
  begin
    ...
    MainForm.TryToSendData('SAYBATTLEEX ' + 'suggests ' + Utility.MapList[(Sender as TSpTBXItem).Tag]);
    Exit;
  end;
  ...
  ChangeMap((Sender as TSpTBXItem).Tag);
...
begin
  ...
  MinimapZoomedForm.Caption := 'Minimap (' + Utility.MapList[CurrentMapIndex] + ')';
