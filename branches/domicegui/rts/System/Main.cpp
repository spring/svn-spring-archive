...
// Domipheus 02/09/06
// This can be cleaned up. CEGUI handlers will return true if the input was used by the system.
// if it is not used we can forward it into the old/current gui stuff.
// CEGUI testing faff
...
int SpringApp::Run (int argc, char *argv[])
{
	...
	CEGUI::OpenGLRenderer* guiRenderer = new CEGUI::OpenGLRenderer(0, screenWidth, screenHeight);

	CEGUI::LuaScriptModule* script_module = new CEGUI::LuaScriptModule();
	new CEGUI::System( guiRenderer, 0,0,reinterpret_cast<CEGUI::ScriptModule*>(script_module) );
	...
	double ceGuiLastTimePulse = 0.001*static_cast<double>(SDL_GetTicks());
	...
	/*
		...
		Below is pretty nasty - its been simply cut and pasted from cegui tutorials,
		But it has not been integrated with the usual spring handling stuff. This can be refactored
		quite easily. There is a cegui windowed bug where it doesnt see the correct mouse pos which
		needs fixed.
	*/
      ...
      case SDL_QUIT:
        ...
        break;
			...
			switch (e.type) {
				...
				case SDL_MOUSEBUTTONUP:
				case SDL_SYSWMEVENT:
					mouseInput->HandleSDLMouseEvent (e);
					break;
				case SDL_KEYDOWN:
				{
					...
					if(activeController) {
						int j = tolower(e.key.keysym.unicode);
		...
		inject_time_pulse(ceGuiLastTimePulse);
