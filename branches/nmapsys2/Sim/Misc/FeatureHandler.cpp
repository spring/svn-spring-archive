...
struct CFeatureDrawCallback : public IVisibilityNodeDrawer
{
	void Draw(VisibilityNode *node) {
		featureHandler->featureDrawList.push_back(static_cast<CFeature*>(node->userData));
	}
} static featureDrawCallback;
...
START_TIME_PROFILE
	while(!toBeRemoved.empty()){
		...
		if(feature){
			...
			if (feature->visibilityNode)
				readmap->GetGroundDrawer()->UnregisterVisibilityNode(feature->visibilityNode);
			delete feature;
		}
	}
...
int CFeatureHandler::AddFeature(CFeature* feature)
{
	...
	// only feature that have a 3d model need to be displayed
	if (feature->def->drawType==DRAWTYPE_MODEL) {
		feature->visibilityNode = readmap->GetGroundDrawer()->RegisterVisibilityNode(feature->pos, feature->radius, &featureDrawCallback);
		feature->visibilityNode->userData = feature;
	}
	return ret;
}
...
void CFeatureHandler::LoadFeaturesFromMap(bool onlyCreateDefs)
{
	...
	for(int a=0;a<numType;++a){
		...
		if(!STRCASECMP(name.substr(0,8).c_str(),"TreeType"))
		{
			...
			fd->mass=20;
			featureDefs[name]=fd;
		} else if(!STRCASECMP(name.substr(0,7).c_str(), "GeoVent")){
			FeatureDef* fd=new FeatureDef;
			fd->blocking=0;
			fd->burnable=0;
			fd->destructable=0;
...
{
	for (std::vector<CFeature*>::iterator i = featureDrawList.begin(); i != featureDrawList.end(); ++i)
	{
		CFeature *f = *i;
		FeatureDef* def=f->def;

		if((f->allyteam==-1 || f->allyteam==gu->myAllyTeam || loshandler->InLos(f->pos,gu->myAllyTeam) || gu->spectating) && def->drawType==DRAWTYPE_MODEL){
			if(water && water->drawReflection){
				float3 zeroPos;
				if(f->midPos.y<0){
					zeroPos=f->midPos;
				}else{
