...
CBFGroundDrawer::~CBFGroundDrawer(void)
{
	...
	for (int a=0;a<NUM_VIS_LAYERS;a++) 
		delete[] layers[a].nodes;
...
void CBFGroundDrawer::CalcLayerVisibility (NodeLayer *layer)
{
	float maxdist=layer->maxdist;
	int quadSize = layer->size;
	int cx=(int)(camera->pos.x/(SQUARE_SIZE*quadSize));
	int cy=(int)(camera->pos.z/(SQUARE_SIZE*quadSize));

	int drawSquare=int(maxdist/(SQUARE_SIZE*quadSize))+1;

	const int extraSize=0;
	int drawQuadsX = map->header.mapx / quadSize;
	int drawQuadsY = map->header.mapy / quadSize;

	int sy=cy-drawSquare;
	if(sy<0)
		sy=0;
	int ey=cy+drawSquare;
	if(ey>=map->header.mapy/quadSize)
		ey=map->header.mapy/quadSize-1;
	...
	for(int y=sy;y<=ey;y++){
		...
		for(int x=sx;x<=ex;x++)
		{
			std::vector<VisNode*> nodes = layer->nodes[y*layer->h+x];
			for(std::vector<VisNode*>::iterator o = nodes.begin(); o != nodes.end(); ++o) {
				(*o)->isVisible = true;
				(*o)->callback->Draw(*o);
			}
		}
...
void CBFGroundDrawer::SetupTextureUnits(bool drawReflection,unsigned int overrideVP)
{
	...
	if(DrawExtraTex()){
		...
		float3 ac=map->ambientColor*(210.0/255.0);
...
VisibilityNode* CBFGroundDrawer::RegisterVisibilityNode (const float3& pos, float range, IVisibilityNodeDrawer* cb)
{
	VisNode *n=new VisNode;
	n->callback = cb;
	n->layer = 0;

	range/=SQUARE_SIZE;

	for (int l=0;l<NUM_VIS_LAYERS;l++) 
		if (range <= layers[l].size) {
			n->layer = &layers[l];
			break;
		}
		// put it on the last (biggest)
	if (!n->layer)
		n->layer = &layers[NUM_VIS_LAYERS-1];

	// TODO: register in multiple nodes when overlapping, 
	// or optimize for the fact that they aren't inserted multiple times
	int x = 0.5f + pos.x * (1.0f / (SQUARE_SIZE * n->layer->size));
	int z = 0.5f + pos.z * (1.0f / (SQUARE_SIZE * n->layer->size));
	x = n->layerPos.x = std::max(0, std::min(n->layer->w-1,x));
	z = n->layerPos.y = std::max(0, std::min(n->layer->h-1,z));
	n->layer->nodes[z*n->layer->w+x].push_back(n);

	return n;
}

void CBFGroundDrawer::UnregisterVisibilityNode (VisibilityNode *node)
{
	VisNode *n = static_cast<VisNode*>(node);
	NodeLayer *layer = n->layer;
	std::vector<VisNode*>* nodes = &layer->nodes[n->layerPos.y*layer->w+n->layerPos.x];
	nodes->erase(std::find(nodes->begin(),nodes->end(),n));
	delete n;
}

void CBFGroundDrawer::InitVisNodeLayers()
{
	layers[0].size = 16; // blockMapSize
	layers[1].size = 32; // DRAW_QUAD_SIZE
	layers[2].size = 64; // TREE_SQUARE_SIZE
	assert (NUM_VIS_LAYERS==3);

	for (int a=0;a<NUM_VIS_LAYERS;a++) {
		layers[a].w = map->width/layers[a].size;
		layers[a].h = map->height/layers[a].size;
		layers[a].nodes = new std::vector<VisNode*>[layers[a].w * layers[a].h];
		layers[a].maxdist = 50000 + 3000 * a;
	}
}
