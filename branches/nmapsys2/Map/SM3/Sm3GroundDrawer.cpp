...
struct Sm3VisibilityNode : public VisibilityNode
{
	int nodeLinkIndex;
	terrain::TQuad *quad;
};
...
VisibilityNode *CSm3GroundDrawer::RegisterVisibilityNode (const float3& pos, float range, IVisibilityNodeDrawer* cb)
{
	terrain::TQuad *qtree = tr->GetQuadTree();
	terrain::TQuad *quad = qtree->FindSmallestContainingQuad2D(pos, range, maxQuadDepth);
	if (!quad) quad = qtree;

	Sm3VisibilityNode *node = new Sm3VisibilityNode;
	node->nodeLinkIndex = (int)quad->nodeLinks.size();
	node->quad = quad;
	node->callback = cb;
	quad->nodeLinks.push_back(node);
	return node;
}

void CSm3GroundDrawer::UnregisterVisibilityNode (VisibilityNode *visNode)
{
	Sm3VisibilityNode* node = static_cast<Sm3VisibilityNode*>(visNode);
	terrain::TQuad *q = node->quad;
	if (node->nodeLinkIndex < q->nodeLinks.size()-1) {
		q->nodeLinks.back()->nodeLinkIndex = node->nodeLinkIndex;
		std::swap(q->nodeLinks[node->nodeLinkIndex], q->nodeLinks.back());
	}
	q->nodeLinks.pop_back();
	delete node;
}
