 ...
 */
GLXPBuffer::GLXPBuffer(int requires, const int w, const int h, const bool sharedcontext, const bool shareobjects): m_pDisplay(0), m_glxPbuffer(0), m_glxContext(0), m_pOldDisplay(0), m_glxOldDrawable(0), m_glxOldContext(0), shadowMapSize(shadowMapSize), m_bSharedContext(sharedcontext), m_bShareObjects(shareobjects)
{
	if (!GLX_SGIX_pbuffer)
		return;
	...
	m_glxPbuffer = glXCreateGLXPbufferSGIX(pDisplay, glxConfig[0], w, h, pb_attr);
	if (!m_glxPbuffer)
		return;
	if (m_bSharedContext)
