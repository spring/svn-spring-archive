...
CBasicTreeDrawer::CBasicTreeDrawer()
{
	...
	for(int y=0;y<treesY;y++){
		for(int x=0;x<treesX;x++){
			...
			float3 position((x+0.5f)*TREE_SQUARE_SIZE*SQUARE_SIZE,0.0f,(y+0.5f)*TREE_SQUARE_SIZE*SQUARE_SIZE);
			position.y = ground->GetHeight(position.x,position.z);
			tss->visibilityNode = gd->RegisterVisibilityNode(position, TREE_SQUARE_SIZE*SQUARE_SIZE/2, &drawCallback);
			tss->visibilityNode->userData = tss;
		}
	}
	drawCallback.me=this;
}
...
CBasicTreeDrawer::~CBasicTreeDrawer()
{
	...
	for(int y=0;y<treesY;y++){
		for(int x=0;x<treesX;x++){
			...
			if(trees[y*treesX+x].visibilityNode)
				readmap->GetGroundDrawer()->UnregisterVisibilityNode(trees[y*treesX+x].visibilityNode);
		}
	}
	delete[] trees;
}
...
void CBasicTreeDrawer::DrawCallback::Draw(VisibilityNode *node)
{
	me->squareDrawList.push_back(static_cast<TreeSquareStruct*>(node->userData));
}
...
void CBasicTreeDrawer::Draw(float treeDistance,bool drawReflection)
{
	...
	for (std::vector<TreeSquareStruct*>::iterator i=squareDrawList.begin();i!=squareDrawList.end();++i) {
		TreeSquareStruct* tss=*i;

		float3 dif;
		dif.x=camera->pos.x-(tss->x*SQUARE_SIZE*TREE_SQUARE_SIZE + SQUARE_SIZE*TREE_SQUARE_SIZE/2);
		dif.y=0;
		dif.z=camera->pos.z-(tss->y*SQUARE_SIZE*TREE_SQUARE_SIZE + SQUARE_SIZE*TREE_SQUARE_SIZE/2);
