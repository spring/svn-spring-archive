...
#include <direct.h>
#include <SDL_syswm.h>
#include <Console/createConsole.h>
#endif
...
{
	ENTER_SYNCED;
	//startscript = "Game/StartScripts/AirScript.cpp";
	if (!startscript.empty()) 
		{
		printf("Setting up new gameSetup\n");
		...
		while (SDL_PollEvent(&event)) {
			switch (event.type) {
...
#endif
					CEGUI::System::getSingleton().injectKeyDown(SDLKeyToCEGUIKey((SDLKey)event.key.keysym.sym));
				...
				case SDL_KEYUP:
				{
					...
					CEGUI::System::getSingleton().injectKeyUp(SDLKeyToCEGUIKey((SDLKey)event.key.keysym.sym));
        			break;
				}
#ifdef WIN32
				case SDL_SYSWMEVENT:
