...
void MessageBox(void* hWnd, const char* lpTest, const char* lpCaption, unsigned int uType)
{
	CLog::Error("Message box - \"%s\" with caption \"%s\"", __FUNCTION__, lpTest, lpCaption); 
}
...
//				MessageBox(0,"Overwrote earlier section in sunparser",thissection.c_str(),0);
			...
			buf = ParseSection(buf, (int)(endptr-buf), section);
	...
	while(buf!=endptr)
	{
		...
		else if(*buf == '{') //section
		{
			...
			if(ui!=section->sections.end()){
				MessageBox((void*)0,"Overwrote earlier section in sunparser",thissection.c_str(),0);
				DeleteSection(&ui->second->sections);
				delete ui->second;
			}
			section->sections[thissection] = newsection;
			buf = ParseSection(buf, (int)(endptr-buf), newsection);
		}
