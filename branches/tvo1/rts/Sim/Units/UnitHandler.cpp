...
#include "Sim/Units/ResyncClasses.h"
#include "mmgr.h"
...
{
	CChecksum checksum;
	checksum.r = gs->randSeed;

	list<CUnit*>::const_iterator usi;
	for(usi=activeUnits.begin();usi!=activeUnits.end();usi++){
		checksum.x ^= makeChecksum((*usi)->midPos.x);
		checksum.y ^= makeChecksum((*usi)->midPos.y);
		checksum.z ^= makeChecksum((*usi)->midPos.z);
	}
	...
	for(int a=0;a<gs->activeTeams;++a){
		checksum.m ^= makeChecksum(gs->Team(a)->metal);
		checksum.e ^= makeChecksum(gs->Team(a)->energy);
	}
#ifdef TRACE_SYNC
...
void CUnitHandler::SendUnitChecksums()
{
	std::vector<unsigned short> temp;
	list<CUnit*>::const_iterator usi;
	for(usi=activeUnits.begin();usi!=activeUnits.end();usi++){
		int checksum = CResyncUnit::getChecksum(*usi);
		temp.push_back((*usi)->id);
		temp.push_back((checksum ^ (checksum >> 16)) & 0xFFFF);
	}
	net->SendSTLData<unsigned char, int, std::vector<unsigned short> >(
			NETMSG_CHECKSUMRESPONSE, gu->myPlayerNum, gs->frameNum, temp);
}

void CUnitHandler::SendResyncResponse(const std::vector<unsigned short>& unitsToResync)
{
	for (int a = 0; a < unitsToResync.size(); ++a) {
		assert(unitsToResync[a] < MAX_UNITS);
		CUnit* u = units[unitsToResync[a]];
		if (u != NULL) {
			// Send a separate message for each unit. The overhead is minimal anyway because of sizeof CResyncUnit
			net->SendData<int, CResyncUnit>(NETMSG_RESYNCRESPONSE, 0 /* frameNum (set by server) */, CResyncUnit(*u));
		} else
			info->AddLine("Got resync request for non existant unit");
	}
}

void CUnitHandler::ReceiveResyncResponse(const CResyncUnit* ru)
{
	assert(ru->getID() >= 0 && ru->getID() < MAX_UNITS);
	CUnit* u = units[ru->getID()];
	if (u != NULL) {
		ru->copyToCUnit(u);
		info->AddLine("Resynced %s of %s", u->unitDef->name.c_str(), gs->players[u->team]->playerName.c_str());
	} else
		info->AddLine("Got resync response for non existant unit");
}
