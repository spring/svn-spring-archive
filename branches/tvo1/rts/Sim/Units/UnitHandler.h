...
struct CChecksum {
	CChecksum() : r(0), x(0), y(0), z(0), m(0), e(0) {}
	int toInt() const                          { return r ^ x ^ y ^ z ^ m ^ e; }
	bool operator<(const CChecksum& c) const   { return toInt() < c.toInt(); }
	operator bool() const                      { return toInt() != 0; }
	bool operator==(const CChecksum& c) const  { return r == c.r && x == c.x && y == c.y && z == c.z && m == c.m && e == c.e; }
	bool operator!=(const CChecksum& c) const  { return !(*this == c); }
	CChecksum& operator=(int a)                { r = x = y = z = m = e = a; return *this; }
	char* diff(char* buf, const CChecksum& c);
	int r, x, y, z, m, e; // randSeed, midPos.x, midPos.y, midPos.z, metal, energy
};
...
public:
	...
	void SendUnitChecksums();
	void SendResyncResponse(const std::vector<unsigned short>& unitsToResync);
	void ReceiveResyncResponse(const CResyncUnit* ru);
	void Update();
