...
//					(*wi)->HoldFire();
		...
		case NETMSG_CHECKSUMREQUEST: {
			int frame=*((int*)&inbuf[inbufpos+1]);
			if(frame != gs->frameNum) {
				info->AddLine("Checksum request for wrong frame (%i instead of %i)", frame, gs->frameNum);
			} else
				uh->SendUnitChecksums();
			lastLength=5;
			break;
		}
		...
		case NETMSG_RESYNCREQUEST:
			lastLength=*((short int*)&inbuf[inbufpos+1]);
			if (gu->myPlayerNum == inbuf[3]) {
				int numelem = (lastLength - 4) / sizeof(short);
				unsigned short* begin = (unsigned short*)&inbuf[inbufpos+4];
				unsigned short* end = begin + numelem;
				std::vector<unsigned short> unitsToResync(numelem);
				std::copy(begin, end, unitsToResync.begin());
				uh->SendResyncResponse(unitsToResync);
			}
			break;
		...
		case NETMSG_RESYNCRESPONSE: {
			int frame=*((int*)&inbuf[inbufpos+1]);
			if (frame != gs->frameNum) {
				info->AddLine("Resync response for wrong frame (%i instead of %i)", frame, gs->frameNum);
			} else {
				CResyncUnit* ru = (CResyncUnit*)&inbuf[inbufpos+5];
				uh->ReceiveResyncResponse(ru);
			}
