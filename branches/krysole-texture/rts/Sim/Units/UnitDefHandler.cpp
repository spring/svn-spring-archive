...
CUnitDefHandler::~CUnitDefHandler(void)
{
	...
	for(int i = 1; i <= numUnits; i++){
		...
		if (unitDefs[i].unitimage) {
			unitDefs[i].unitimage = Texture(); // Will deallocate texture by force here
		}
	}
	delete[] unitDefs;
...
Texture CUnitDefHandler::GetUnitImage(UnitDef *unitdef)
{
	return unitdef->unitimage ? unitdef->unitimage : 
		LoadUnitImage(unitdef);
}

Texture CUnitDefHandler::LoadUnitImage(UnitDef *unitdef)
{
	Texture tex;
	
	// If we have a buildpicname then we'll try that first
	if (!unitdef->buildpicname.empty()) {
		tex = TextureLoader::Instance().
			LoadTexture("unitpics/" + unitdef->buildpicname, false);
	}
	
	// Next try the unit name with a pcx extension
	if (!tex) {
		tex = TextureLoader::Instance().
			LoadTexture("unitpics/" + unitdef->name + ".pcx", false);
	}
	
	// Finally try the unit name with a bmp extension
	if (!tex) {
		tex = TextureLoader::Instance().
			LoadTexture("unitpics/" + unitdef->name + ".bmp", false);
	}
	
	// Failing all that create a dummy texture and 
	// indicate the missing resource on the cmdline
	if (!tex) {
		tex = mtxeffect::Create1x1Texture(1, 1, 1, 1);
		fprintf(stderr, "Warning: Build Pic not found!\n\tUnit Name: %s", unitdef->name.c_str());
	}
	
	return unitdef->unitimage = tex;
}
