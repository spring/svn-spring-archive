...
CProjectileHandler::CProjectileHandler()
{
	...
	if(shadowHandler && shadowHandler->drawShadows && GLEW_EXT_framebuffer_object && !GLEW_ATI_envmap_bumpmap){	//this seems to bug on ati cards so disable it on those (just some random ati extension to detect ati cards), should be fixed by someone that actually has a ati card
		...
		if (perlinFB && perlinFB->valid()){
			drawPerlinTex=true;
			perlinFB->attachTexture(textureAtlas->gltex, FBO_ATTACH_COLOR);
...
{
	MemTexture rgbTexture = TextureLoader::Instance().LoadMemTexture(filename);
	MemTexture alphaTexture = TextureLoader::Instance().LoadMemTexture(alphafile);
	const unsigned char *rgbmem = rgbTexture.Bytes();
	const unsigned char *alphamem = alphaTexture.Bytes();
	for(int y=0;y<32;y++){
		for(int x=0;x<32;x++){
			tex[yoffs+y][xoffs+x][0]=rgbmem[(y*32+x)*4];
			tex[yoffs+y][xoffs+x][1]=rgbmem[(y*32+x)*4+1];
			tex[yoffs+y][xoffs+x][2]=rgbmem[(y*32+x)*4+2];
			tex[yoffs+y][xoffs+x][3]=alphamem[(y*32+x)*4];
		}
	}
}
