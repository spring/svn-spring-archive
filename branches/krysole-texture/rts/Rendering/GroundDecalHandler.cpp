...
void CGroundDecalHandler::Draw(void)
{
	...
	if(shadowHandler && shadowHandler->drawShadows){
		...
		shadowHandler->shadowTexture.Bind();
...
Texture CGroundDecalHandler::LoadTexture(std::string name)
{
	...
	MemTexture mtx = TextureLoader::Instance().LoadMemTexture(name);

	// TODO: mtxeffect: Some Sort of Gamma or Brightness Function

	unsigned int width = mtx.Width();
	unsigned int height = mtx.Height();
	unsigned char *bytes = mtx.Bytes();

	for(int y=0;y<height;++y){
		for(int x=0;x<width;++x){
			bytes[(y*width+x)*4+3]=bytes[(y*width+x)*4+1];
			int brighness=bytes[(y*width+x)*4+0];
			bytes[(y*width+x)*4+0]=(brighness*90)/255;
			bytes[(y*width+x)*4+1]=(brighness*60)/255;
			bytes[(y*width+x)*4+2]=(brighness*30)/255;
		}
	}
	
	return mtx.CreateTexture(true);
}
...
{
	MemTexture mtx = TextureLoader::Instance().LoadMemTexture(file);
	for(int y=0;y<mtx.Height();++y){
		for(int x=0;x<mtx.Width();++x){
			buf[((y+yoffset)*512+x+xoffset)*4+3]=mtx.Bytes()[(y*mtx.Width()+x)*4+1];
			int brighness=mtx.Bytes()[(y*mtx.Width()+x)*4+0];
...
int CGroundDecalHandler::GetBuildingDecalType(std::string name)
{
	...
	tt->name=name;
	tt->texture = TextureLoader::Instance().LoadTexture("unittextures/" + name, true);
