...
CDynWater::CDynWater(void)
{
	...
	MemTexture foam = TextureLoader::Instance().LoadMemTexture("bitmaps/foam.jpg");
	for(int a=0;a<256*256;++a)
		scrap[a]=foam.Bytes()[a*4];
	...
	mtx = TextureLoader::Instance().LoadMemTexture("bitmaps\\boatshape.bmp");
	bytes = mtx.Bytes();
	pixels = mtx.Total();
	for(int a = 0; a < pixels; ++a){
		bytes[a * 4 + 2] = bytes[a * 4 + 0];
		bytes[a * 4 + 3] = bytes[a * 4 + 0];
	}
	boatShape = mtx.CreateTexture(false);

	mtx = TextureLoader::Instance().LoadMemTexture("bitmaps\\hovershape.bmp");
	bytes = mtx.Bytes();
	pixels = mtx.Total();
	for(int a=0;a<pixels;++a){
		bytes[a*4+2]=bytes[a*4];
		bytes[a*4+3]=bytes[a*4];
	}
	hoverShape=mtx.CreateTexture(false);
