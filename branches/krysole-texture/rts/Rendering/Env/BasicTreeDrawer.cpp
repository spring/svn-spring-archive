...
CBasicTreeDrawer::CBasicTreeDrawer()
{
	...
	MemTexture mtx = TextureLoader::Instance().LoadMemTexture(
		"bitmaps/"+resources.SGetValueDef("gran.bmp","resources\\graphics\\trees\\gran1"));
	mtxeffect::Flip(mtx);
	...
	if (mtx.Width()>1){
		for(int y=0;y<256;y++){
			for(int x=0;x<256;x++){
				if(mtx.Bytes()[(y*256+x)*4]==72 && mtx.Bytes()[(y*256+x)*4+1]==72){
					gran[y][x][0]=33;
					gran[y][x][1]=54;
					gran[y][x][2]=29;
					gran[y][x][3]=0;
				} else {
					gran[y][x][0]=mtx.Bytes()[(y*256+x)*4];
					gran[y][x][1]=mtx.Bytes()[(y*256+x)*4+1];
					gran[y][x][2]=mtx.Bytes()[(y*256+x)*4+2];
					gran[y][x][3]=255;
				}
			}
		}
	...
	mtx = TextureLoader::Instance().LoadMemTexture(
		"bitmaps/"+resources.SGetValueDef("gran2.bmp","resources\\graphics\\trees\\gran2"));
	mtxeffect::Flip(mtx);
	if (mtx.Width()>1){
		for(int y=0;y<256;y++){
			for(int x=0;x<256;x++){
				if(mtx.Bytes()[(y*256+x)*4]==72 && mtx.Bytes()[(y*256+x)*4+1]==72){
					...
					gran[y][x+256][0]=mtx.Bytes()[(y*256+x)*4];
					gran[y][x+256][1]=mtx.Bytes()[(y*256+x)*4+1];
					gran[y][x+256][2]=mtx.Bytes()[(y*256+x)*4+2];
					gran[y][x+256][3]=255;
				}
			}
		}
	...
	mtx = TextureLoader::Instance().LoadMemTexture(
		"bitmaps/"+resources.SGetValueDef("birch1.bmp","resources\\graphics\\trees\\birch1"));
	mtxeffect::Flip(mtx);
	if (mtx.Width()>1){
		for(int y=0;y<256;y++){
			for(int x=0;x<128;x++){
				if(mtx.Bytes()[(y*128+x)*4]==72 && mtx.Bytes()[(y*128+x)*4+1]==72){
					...
					gran[y+256][x][0]=(unsigned char)(mtx.Bytes()[(y*128+x)*4]*0.6f);
					gran[y+256][x][1]=(unsigned char)(mtx.Bytes()[(y*128+x)*4+1]*0.7f);
					gran[y+256][x][2]=(unsigned char)(mtx.Bytes()[(y*128+x)*4+2]*0.6f);
					gran[y+256][x][3]=255;
				}
			}
		}
	...
	mtx = TextureLoader::Instance().LoadMemTexture(
		"bitmaps/"+resources.SGetValueDef("birch2.bmp","resources\\graphics\\trees\\birch2"));
	mtxeffect::Flip(mtx);
	if (mtx.Width()>1){
		for(int y=0;y<256;y++){
			for(int x=0;x<128;x++){
				if(mtx.Bytes()[(y*128+x)*4]==72 && mtx.Bytes()[(y*128+x)*4+1]==72){
					...
					gran[y+256][x+128][0]=(unsigned char)(mtx.Bytes()[(y*128+x)*4]*0.6f);
					gran[y+256][x+128][1]=(unsigned char)(mtx.Bytes()[(y*128+x)*4+1]*0.7f);
					gran[y+256][x+128][2]=(unsigned char)(mtx.Bytes()[(y*128+x)*4+2]*0.6f);
	...
	mtx = TextureLoader::Instance().LoadMemTexture(
		"bitmaps/"+resources.SGetValueDef("birch3.bmp","resources\\graphics\\trees\\birch3"));
	mtxeffect::Flip(mtx);
	if (mtx.Width()>1){
		for(int y=0;y<256;y++){
			for(int x=0;x<256;x++){
				if(mtx.Bytes()[(y*256+x)*4]==72 && mtx.Bytes()[(y*256+x)*4+1]==72){
					...
					gran[y+256][x+256][0]=(unsigned char)(mtx.Bytes()[(y*256+x)*4]*0.6f);
					gran[y+256][x+256][1]=(unsigned char)(mtx.Bytes()[(y*256+x)*4+1]*0.7f);
					gran[y+256][x+256][2]=(unsigned char)(mtx.Bytes()[(y*256+x)*4+2]*0.6f);
