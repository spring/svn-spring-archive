...
CAdvTreeGenerator::CAdvTreeGenerator()
{
	...
	MemTexture mtx = TextureLoader::Instance().LoadMemTexture(
		"bitmaps/"+resources.SGetValueDef("Bark.bmp","resources\\graphics\\trees\\bark"));
	for(int y=0;y<256;y++){
		for(int x=0;x<256;x++){
			tree[y][x][0]=mtx.Bytes()[(y*256+x)*4];
			tree[y][x][1]=mtx.Bytes()[(y*256+x)*4+1];
			tree[y][x][2]=mtx.Bytes()[(y*256+x)*4+2];
			tree[y][x][3]=255;
		}
	}
	
	mtx = TextureLoader::Instance().LoadMemTexture(
		"bitmaps/"+resources.SGetValueDef("bleaf.bmp","resources\\graphics\\trees\\leaf"));
	mtxeffect::AlphaWithColorKey(mtx, 0, 0, 0);
	//bm.Save("baseleaf.bmp");
	mtxeffect::Renormalize(mtx, float3(0.22f, 0.43f, 0.18f));
//	bm.Save("baseleaf2.bmp");

	Texture leafTex = mtx.CreateTexture(true);
	...
	for(int y=0;y<256;y++){
		for(int x=0;x<256;x++){
			tree[y][x+1024][0]=(unsigned char)(mtx.Bytes()[(y*256+x)*4]*0.6f);
			tree[y][x+1024][1]=(unsigned char)(mtx.Bytes()[(y*256+x)*4+1]*0.6f);
			tree[y][x+1024][2]=(unsigned char)(mtx.Bytes()[(y*256+x)*4+2]*0.6f);
			tree[y][x+1024][3]=255;
		}
	}
...
void CAdvTreeGenerator::CreateLeafTex(const Texture &baseTex, int xpos, int ypos,unsigned char buf[256][2048][4])
{
