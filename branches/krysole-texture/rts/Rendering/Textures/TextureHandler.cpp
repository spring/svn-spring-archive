...
static int CompareTatex2( const void *arg1, const void *arg2 ){
	if((*(TexFile**)arg1)->tex.Height() > (*(TexFile**)arg2)->tex.Height())
	   return -1;
   return 1;
}
...
CTextureHandler::CTextureHandler()
{
	...
	for(vector<string>::iterator fi=files.begin();fi!=files.end();++fi){
		...
		if(teamTexes.find(s2)==teamTexes.end()){
			...
			tex->tex = TextureLoader::Instance().LoadMemTexture(s);
			mtxeffect::SetAlpha(tex->tex, 30); // Change alpha to 30 instead of 255
			tex->name=s2;	
			texfiles[numfiles++]=tex;
			totalSize+=tex->tex.Width()*tex->tex.Height();
		} else {
	...
	for(int a=0;a<256;++a){
		...
		TexFile* tex=new TexFile;
		tex->name=name;
		tex->tex = mtxeffect::Create1x1MemTexture(palette[a][0], 
			palette[a][1], palette[a][2], 30);
	...
	for(int a=0;a<numfiles;++a){
		MemTexture &curtex = texfiles[a]->tex; // Shortcut name
		...
		while(!done){
			...
			if(thisSub.front().x+curtex.Width()>bigTexX){
				thisSub.clear();
				continue;
			}
			if(thisSub.front().y+curtex.Height()>maxy){
				thisSub.pop_front();
				continue;
			}
			...
			if(thisSub.front().y+curtex.Height()<maxy){
				nextSub.push_back(int2(thisSub.front().x,thisSub.front().y+curtex.Height()));
			}

			thisSub.front().x+=curtex.Width();
		...
		for(int y=0;y<curtex.Height();y++){
			for(int x=0;x<curtex.Width();x++){
...
//				} else {
					for(int col=0;col<4;col++){
						tex[((foundy+y)*bigTexX+(foundx+x))*4+col]=curtex.Bytes()[(y*curtex.Width()+x)*4+col];
//					}
				}
			}
		}
		...
		unittex->xend=(foundx+curtex.Width()-0.5f)/(float)bigTexX;
		unittex->yend=(foundy+curtex.Height()-0.5f)/(float)bigTexY;
...
int CTextureHandler::LoadS3OTexture(string tex1, string tex2)
{
	...
	tex.tex1 = TextureLoader::Instance().
		LoadTexture("unittextures/"+tex1, true);
	tex.tex2=Texture();
	if(unitDrawer->advShading){
		tex.tex2 = TextureLoader::Instance().
			LoadTexture("unittextures/"+tex2, true);
	}
...
{
	s3oTextures[num].tex1.Bind();
	if(unitDrawer->advShading){
		glActiveTextureARB(GL_TEXTURE1_ARB);
		s3oTextures[num].tex2.Bind();
...
{
	TexFile* tex=new TexFile;
	tex->tex = TextureLoader::Instance().LoadMemTexture(name);
	mtxeffect::SetAlpha(tex->tex, 30);
	char tmp[256];
	sprintf(tmp,"%s%02i",name2.c_str(),team);
	tex->name=tmp;

	unsigned char* teamCol=gs->Team(team)->color;
	MemTexture &mtx = tex->tex; // shortcut name
	unsigned char *mem = mtx.Bytes();
	for (int a = 0; a < mtx.Height() * mtx.Width(); ++a) {
		if (mem[a * 4] == mem[a * 4 + 2] && mem[a * 4 + 1] == 0) {
			float lum = mem[a * 4] / 255.0f;
			mem[a * 4 + 0] = (unsigned char)(min(255, int(teamCol[0] * lum * 1.5)));
			mem[a * 4 + 1] = (unsigned char)(min(255, int(teamCol[1] * lum * 1.5)));
			mem[a * 4 + 2] = (unsigned char)(min(255, int(teamCol[2] * lum * 1.5)));
		}	
	}
	return tex;
}
