...
#ifndef SPRING_RTS_MEM_TEXTURE_H
#define SPRING_RTS_MEM_TEXTURE_H
...
{
public:
	MemTexture(); // Asserts are used to prevent accessing an uninitialized object (does not check if NDEBUG)
	...
	MemTexture(const unsigned char *mem, std::size_t width, std::size_t height, 
		bool takeMemory = false);
	MemTexture(std::size_t width, std::size_t height);
...
private:
	boost::shared_ptr<MemTextureImpl> pimpl_;
};

#endif /* SPRING_RTS_MEM_TEXTURE_H */
