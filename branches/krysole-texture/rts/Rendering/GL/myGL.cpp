...
void LoadStartPicture()
{
	...
	if (mtx.Width() > gu->screenx || mtx.Height() > gu->screeny)
	{
		mtx = mtxeffect::ScaleToSize(mtx, gu->screenx, gu->screeny);
	}

	startupTexture = mtx.CreateTexture(false);
}

void UnloadStartPicture()
{
	startupTexture = Texture();
}
