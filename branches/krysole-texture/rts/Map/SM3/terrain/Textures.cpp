...
//-----------------------------------------------------------------------
	...
	void Blendmap::Load(const std::string& name, const std::string& section, Heightmap *heightmap, ILoadCallback *cb, TdfParser *tdf)
	{
		...
		if (generateBlendmap) {
			...
			MemTexture mtx = TextureLoader::Instance().LoadMemTexture(blendmapImg);
			if (!mtx)
	...
	Texture LoadTexture (const string& fn, bool isBumpmap)
	{
		Texture tex = TextureLoader::Instance().LoadTexture(fn, true);
		if (!tex)
