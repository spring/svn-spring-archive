...
Texture LoadTexture(const std::string& name, const vector<paletteentry_s>* pvTransparentColors)
{
	...
	if ( pvTransparentColors != NULL )
	{
		...
		{
			mtxeffect::AlphaWithColorKey(mtx, currIter->peRed, currIter->peGreen, currIter->peBlue);
		}
	}
	return mtx.CreateTexture(false);
};
