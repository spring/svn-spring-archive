...
//		hideInterface=!hideInterface;
	...
	//	(*guicontroller) << "Cloud density " << 1/sky->cloudDensity << "\n";
		...
		// Make sure that the screen width is on a 32 bit boundary (keep old value)
		int x=gu->screenx;
		if(gu->screenx%4)
			gu->screenx+=4-gu->screenx%4;

		MemTexture mtx(gu->screenx, gu->screeny);
		glReadPixels(0, 0, mtx.Width(), mtx.Height(), GL_RGBA, GL_UNSIGNED_BYTE, mtx.Bytes());

		// Flip image right way up (so that its not upside-down
		...
		// Create filename string, date-time is used to ensure unique, sorted screenshots
		// without the overhead of maintaining a counter or performing a file listing
		std::stringstream filename;
		boost::posix_time::ptime pt = boost::posix_time::microsec_clock::local_time();
		filename << "screenshots/" << boost::posix_time::to_iso_extended_string(pt) << ".jpg";
		// NOTE: this will produce a filename in the following format
		// screenshots/YYYYMMDDTHHMMSS,fffffffff.jpg

		TextureLoader::Instance().SaveMemTexture(mtx, filename.str()); 

		// Restore old screen width before stride calculation
		gu->screenx=x;
	}
