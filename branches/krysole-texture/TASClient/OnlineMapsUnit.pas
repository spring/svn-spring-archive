...
type
  TOnlineMapsForm = class(TForm)
    ...
    procedure ThreadedLoadOnlineMapsFromCache(DirPath: string);
...
begin
  ...
  try
    ...
    OnlineMapsForm.ThreadedLoadOnlineMapsFromCache(ExtractFilePath(Application.ExeName) + CACHE_FOLDER);
  finally
    ReadingCache := False;
...
procedure TOnlineMapsForm.ThreadedLoadOnlineMapsFromCache(DirPath: string); // Note: you should call it only when program starts and not in the middle of program execution, since it may clash with TUpdateThrd when updating online maps
var
  i: Integer;
  f: TextFile;
  s1, s2, s3, s4: string;
...
begin
  ...
  begin
    ...
    try
      ...
      AddOnlineMap(s1, s2, s3, s4); //*** this should be synchronized with main VCL thread... (also all references to OnlineMaps list!)
